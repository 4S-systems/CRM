...
public class StaffCodeMgr extends RDBGateWay {

    SqlMgr sqlMgr = SqlMgr.getInstance();
    private static StaffCodeMgr staffCodeMgr = new StaffCodeMgr();
...
//    private static final String updatefailureSQL = "UPDATE failure_code SET TITLE = ?, CREATION_TIME = now(), CREATED_BY = ?, DESCRIPTION = ?  WHERE ID = ?";
    ...
    public static StaffCodeMgr getInstance() {
        logger.info("Getting StaffCodeMgr Instance ....");
        return staffCodeMgr;
    }
    ...
    protected void initSupportedForm() {
        ...
        if (supportedForm == null) {
            try {
                supportedForm = new BusinessForm(DOMFabricatorBean.getDocument(metaDataMgr.getMetadata("staff_code.xml")));
            } catch (Exception e) {
    ...
    public boolean saveObject(WebBusinessObject staff, HttpSession s, String crewCode) throws NoUserInSessionException {
        ...
        params.addElement(new StringValue(UniqueIDGen.getNextID()));
        params.addElement(new StringValue((String) staff.getAttribute("code")));
        params.addElement(new StringValue((String) staff.getAttribute("description")));
        params.addElement(new StringValue((String) waUser.getAttribute("userId")));
        params.addElement(new StringValue((String) staff.getAttribute("crewID")));
        params.addElement(new StringValue(crewCode));
        ...
        try {
            ...
            forInsert.setSQLQuery(sqlMgr.getSql("insertStaffCodeSQL").trim());
...
//            if(wbo.getAttribute("isMaintainable").toString().equalsIgnoreCase("1")) {
    ...
    public ArrayList getEmptyCrewCode() {
        cashedData = new ArrayList();
        WebBusinessObject wbo = null;
        cashData();
        for (int i = 0; i < cashedTable.size(); i++) {
            ...
            try {
                ...
                if (wbo.getAttribute("staffFlag").toString().equalsIgnoreCase("0")) {
                    cashedData.add(wbo);
                }
            } catch (Exception ex) {
                logger.error(ex.getMessage());
    ...
    public boolean updateStaffCode(WebBusinessObject failure, HttpSession s, String crewCode) {
        ...
        params.addElement(new StringValue((String) failure.getAttribute("code")));
        params.addElement(new StringValue((String) failure.getAttribute("description")));
        params.addElement(new StringValue((String) waUser.getAttribute("userId")));
        params.addElement(new StringValue((String) failure.getAttribute("crewId")));
        params.addElement(new StringValue(crewCode));
        params.addElement(new StringValue((String) failure.getAttribute("staffCodeId")));
        ...
        try {
            ...
            forUpdate.setSQLQuery(sqlMgr.getSql("updatestaffCodeSQL").trim());
