...
public class GroupPrevMgr extends RDBGateWay {

    private static GroupPrevMgr groupPrevMgr = new GroupPrevMgr();
    ...
    public static GroupPrevMgr getInstance() {
        logger.info("Getting GroupPrevMgr Instance ....");
        return groupPrevMgr;
    }
    ...
    protected void initSupportedForm() {
        ...
        if (supportedForm == null) {
            try {
                supportedForm = new BusinessForm(DOMFabricatorBean.getDocument(metaDataMgr.getMetadata("group_prev.xml")));
            } catch (Exception e) {
    ...
    public boolean saveGroupPreviliges(String groupId, Vector userStores, HttpSession s, String type) {
        ...
        try {
            ...
            forInsert.setSQLQuery(sqlMgr.getSql("deleteOldGroupPrivileges").trim());
            queryResult = forInsert.executeUpdate();

            String sql = sqlMgr.getSql("saveGroupPreviliges").trim();
            forInsert.setSQLQuery(sql);

            for (int i = 0; i < userStores.size(); i++) {

                userStoreWBO = (WebBusinessObject) userStores.get(i);
                if (userStoreWBO.getAttribute("prevType").equals(type)) {
                    params = new Vector();
...
//                    params.addElement(new StringValue(""));
    ...
    public ArrayList<String> getGroupPrivilegeCodes(String userId) {
        ...
        try {
            ...
            forQuery.setSQLQuery(sqlMgr.getSql("getGroupPrivilegeCodes").trim());
        ...
        try {
            while (e.hasMoreElements()) {
                r = (Row) e.nextElement();
                userPrivilegeList.add(r.getString("store_code"));
    ...
    public ArrayList getGroupPrivligesByType(String type, String id) throws SQLException, NoSuchColumnException {
        ...
        try {
            ...
            forQuery.setConnection(connection);
            forQuery.setSQLQuery(sqlMgr.getSql("getGroupPrevliges").trim());
            forQuery.setparams(params);
            queryResult = forQuery.executeQuery();
