...
public class ItemDocMgr extends RDBGateWay {

    SqlMgr sqlMgr = SqlMgr.getInstance();
    private static ItemDocMgr itemDocMgr = new ItemDocMgr();
    private FileMgr fileMgr = FileMgr.getInstance();
    private WebBusinessObject fileDescriptor = null;
    private QueryMgr queryMgr = null;

    public static ItemDocMgr getInstance() {
        logger.info("Getting ItemDocMgr Instance ....");
        return itemDocMgr;
    }
    ...
    protected void initSupportedForm() {
        ...
        if (supportedForm == null) {
            try {
                supportedForm = new BusinessForm(DOMFabricatorBean.getDocument(metaDataMgr.getMetadata("item_doc.xml")));
            } catch (Exception e) {
    ...
    public InputStream getImage(String docId) {
        ...
        try {
            ...
            ps = connection.prepareStatement(sqlMgr.getSql("getItemDocImage").trim());
...
//        java.sql.Timestamp docDate = dropdownDate.getDate(request.getParameter("docDate"));//super.extractDateFromRequest();
        ...
        try {
            ...
            forInsert.setSQLQuery(sqlMgr.getSql("insertItemDocSQL").trim());
...
//        java.sql.Timestamp docDate = dropdownDate.getDate(request.getParameter("docDate"));
        ...
        try {
            ...
            forUpdate.setSQLQuery(sqlMgr.getSql("updateItemDocImage").trim());
    ...
    public WebBusinessObject getOnSingleKey(String key) {
        ...
        String query = new String(sqlMgr.getSql("selectItemDocOnSingleKey").trim());
    ...
    public Vector getListOnLIKE(String forOperation, String keyValue) {
        ...
        try {
            connection = dataSource.getConnection();
            forQuery.setConnection(connection);
            forQuery.setSQLQuery(sqlMgr.getSql("selectItemDocListOnLike").trim());
            forQuery.setparams(SQLparams);
    ...
    public boolean hasDocuments(String itemID) throws Exception {
        ...
        try {
            connection = dataSource.getConnection();
            forQuery.setConnection(connection);
            forQuery.setSQLQuery(sqlMgr.getSql("selectItemDocHasDocument").trim().replace("$", "'" + itemID + "'"));
            queryResult = forQuery.executeQuery();
    ...
    public boolean hasImages(String itemID) throws Exception {
        ...
        try {
            connection = dataSource.getConnection();
            forQuery.setConnection(connection);
            forQuery.setSQLQuery(sqlMgr.getSql("selectItemDocHasImage").trim().replace("$", "'" + itemID + "'"));
            queryResult = forQuery.executeQuery();
    ...
    public Vector getImagesList(String itemID) {
