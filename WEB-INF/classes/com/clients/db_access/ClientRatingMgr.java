...
public class ClientRatingMgr extends RDBGateWay {
            ...
            for (Row row : result) {
                ...
                try {
                    ...
                    if (row.getString("description") != null) {
                        wbo.setAttribute("description", row.getString("description"));
                    } else {
                    ...
                    if (row.getString("MGR_BY") != null) {
                        wbo.setAttribute("MGR_BY", row.getString("MGR_BY"));
                    } else {
                        wbo.setAttribute("MGR_BY", "");
                    }
                    if (row.getString("job") != null) {
                        wbo.setAttribute("job", row.getString("job"));
                    } else {
    ...
    public ArrayList<WebBusinessObject> getClientRateIntervalAllMar(Date fromDate, Date toDate, String seasonTypeFIDs, String campaignID, String sourceListIDs, String projectID, String rateIDs,
            String employeeID, String type, String dateType, String department) {
        ClientComplaintsMgr clientComplaintsMgr = new ClientComplaintsMgr();
        clientComplaintsMgr = ClientComplaintsMgr.getInstance();
        clientComplaintsMgr.updateClientComplaintsType();
        ...
        if (seasonTypeFIDs != null && !seasonTypeFIDs.equals("''") && !seasonTypeFIDs.equals("")) {
            where.append(" AND CL.OPTION3 in ( ").append(seasonTypeFIDs).append(" ) ");
        }
        if (campaignID != null && !campaignID.equals("''") && !campaignID.equals("")) {
            where.append(" AND ccc.campaign_id in ( ").append(campaignID).append(" ) ");
        }
        if (sourceListIDs != null && !sourceListIDs.equals("''") && !sourceListIDs.equals("")) {
            where.append(" AND stk.id in ( ").append(sourceListIDs).append(" ) ");
        }
        if (rateIDs != null && !rateIDs.equals("''")) {
            where.append(" AND CR.RATE_ID IN (").append(rateIDs).append(")");
        }
