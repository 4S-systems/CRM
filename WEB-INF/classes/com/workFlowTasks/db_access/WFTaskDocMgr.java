...
public class WFTaskDocMgr extends RDBGateWay {

    SqlMgr sqlMgr = SqlMgr.getInstance();
    private static WFTaskDocMgr wFTaskDocMgr = new WFTaskDocMgr();
    private FileMgr fileMgr = FileMgr.getInstance();
    private WebBusinessObject fileDescriptor = null;
    private QueryMgr queryMgr = null;

    public static WFTaskDocMgr getInstance() {
        logger.info("Getting wFTaskDocMgr Instance ....");
        return wFTaskDocMgr;
    }
    ...
    protected void initSupportedForm() {
        ...
        if (supportedForm == null) {
            try {
                supportedForm = new BusinessForm(DOMFabricatorBean.getDocument(metaDataMgr.getMetadata("wf_task_document.xml")));
            } catch (Exception e) {
    ...
    public InputStream getImage(String docId) {
        ...
        try {
            ...
            ps = connection.prepareStatement(sqlMgr.getSql("getWfTaskDocImage").trim());
    ...
    public InputStream getWfTaskVisitImage(String docId) {

        PreparedStatement ps = null;
        Connection connection = null;
        try {
            connection = dataSource.getConnection();
            ps = connection.prepareStatement(sqlMgr.getSql("getWfTaskVisitImage").trim());
            ps.setString(1, docId);
    ...
    public boolean saveDocument(HttpServletRequest request, HttpSession session, String filePath) {
        ...
        if (request.getParameter("docType").equalsIgnoreCase("jpg")) {
            try {
                ...
                g.drawString("4S Workflow Ticket", 25, 25);
        ...
        String wfTaskId = (String) request.getParameter("wfTaskId");
        ...
        try {
            ...
            forInsert.setSQLQuery(sqlMgr.getSql("insertWfTaskDocSQL").trim());
    ...
    public boolean saveDocument(WebBusinessObject docData, HttpSession session, String filePath) {
        DropdownDate dropdownDate = new DropdownDate();

        if (docData.getAttribute("docType").toString().equalsIgnoreCase("jpg")) {
            try {
                BufferedImage img = ImageIO.read(new File(filePath));
                int w = img.getWidth();
                int h = img.getHeight();
        ...
        String docBusinessType = (String) docData.getAttribute("docType");
        String docTitle = (String) docData.getAttribute("docTitle");
        String businessObjectId = (String) docData.getAttribute("businessObjectId");
        String objectType = (String) docData.getAttribute("objectType");
        String description = (String) docData.getAttribute("description");
        String total = (String) docData.getAttribute("faceValue");
        String docType = (String) docData.getAttribute("fileExtension");
        ...
        java.sql.Timestamp docDate = dropdownDate.getDate(docData.getAttribute("docDate").toString());//super.extractDateFromRequest();
        ...
        params.addElement(new StringValue(metaType));
        params.addElement(new StringValue(docBusinessType));
        params.addElement(new StringValue(docData.getAttribute("configType").toString()));
//        if (!docData.getAttribute("taskVisitId").toString().equals("1")) {
//            params.addElement(new StringValue(docData.getAttribute("taskVisitId").toString()));
//        }
        
        params.addElement(new StringValue(businessObjectId));
        ...
        try {
            ...
            forInsert.setConnection(connection);
//            if (!docData.getAttribute("taskVisitId").toString().equals("1")) {
//                forInsert.setSQLQuery(sqlMgr.getSql("insertWfTaskVisitDocSQL").trim());
//            } else {
//                forInsert.setSQLQuery(sqlMgr.getSql("insertWfTaskDocSQL").trim());
//            }
            ...
            logger.error(se.getMessage());
            System.out.print("saveDoc..." + se.getMessage());
            return false;
        } finally {
            try {
                dispose = new File(filePath);
    ...
    public boolean updateDocument(HttpServletRequest request, HttpSession session) {
        ...
        try {
            ...
            forUpdate.setSQLQuery(sqlMgr.getSql("updateWFTaskDocImage").trim());
    ...
    public WebBusinessObject getOnSingleKey(String key) {
        ...
        String query = new String(sqlMgr.getSql("selectWFTaskDocOnSingleKey").trim());
    ...
    public Vector getListOnLIKE(String forOperation, String keyValue) {
        ...
        try {
            connection = dataSource.getConnection();
            forQuery.setConnection(connection);
            forQuery.setSQLQuery(sqlMgr.getSql("selectWfTaskDocListOnLike").trim());
            forQuery.setparams(SQLparams);
    ...
    public Vector getListTaskDoc(String keyValue) {
        ...
        try {
            connection = dataSource.getConnection();
            forQuery.setConnection(connection);
            forQuery.setSQLQuery(sqlMgr.getSql("getListTaskDoc").trim());
            forQuery.setparams(SQLparams);
    ...
    public Vector getListTaskVisitDoc(String taskId, String taskVisitId) {
        ...
        SQLparams.addElement(new StringValue(taskVisitId));
        ...
        try {
            connection = dataSource.getConnection();
            forQuery.setConnection(connection);
            forQuery.setSQLQuery(sqlMgr.getSql("getListTaskVisitDoc").trim());
            forQuery.setparams(SQLparams);
    ...
    public String getDocsCount(String wfTaskId) {
        String num = "";
        ...
        try {
            ...
            forSelect.setSQLQuery(sqlMgr.getSql("getNumDocs").trim());
    ...
    public Vector getTaskDocInfo(String wfTaskId) {
        Vector res = new Vector();
        Vector params = new Vector();
        SQLCommandBean forSelect = new SQLCommandBean();
        ...
        try {
            connection = dataSource.getConnection();
            forSelect.setConnection(connection);
            forSelect.setSQLQuery(sqlMgr.getSql("getTaskDocInfo").trim());
            forSelect.setparams(params);
            res = forSelect.executeQuery();
        } catch (Exception se) {
            logger.error(se.getMessage());
    ...
    public InputStream getWfTaskDocByDocId(String DocId) {
        PreparedStatement ps = null;
        Connection connection = null;
        try {
            connection = dataSource.getConnection();
            ps = connection.prepareStatement(sqlMgr.getSql("getWfTaskDocByDocId").trim());
            ps.setString(1, DocId);
