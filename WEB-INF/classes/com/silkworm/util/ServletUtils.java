 ...
 * ServletUtil.java
 *
 * Created on October 24, 2003, 10:42 PM
 */
...
public class ServletUtils {
  ...
  public static ArrayList getRequestParams(HttpServletRequest req)
  {
   String param;
   String value;
   int index = 0;
  ...
  while(e.hasMoreElements())
  {
    ...
    value = "";
            try {
                value = new String(req.getParameter(param).getBytes(), "ISO-8859-1");
 ...
 public static void printRequest(ArrayList reqParams)
  {
   ...
   while(li.hasNext())
   {
    ...
    System.out.println("PARAM NAME: " + di.getItemName() + "  PARAM VALUE: " + di.getItemValue());
   }

  }
 
    public static ArrayList<WebBusinessObject> getApplicationMessages(HttpSession session) {
        ArrayList<WebBusinessObject> messagesList = new ArrayList<>();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        Calendar c = Calendar.getInstance();
        Calendar temp = Calendar.getInstance();
        Date onDate, uptoDate;
        String frequency;
        if (session.getServletContext().getAttribute("messagesList") != null) {
            for (WebBusinessObject messageWbo : (ArrayList<WebBusinessObject>) session.getServletContext().getAttribute("messagesList")) {
                try {
                    frequency = (String) messageWbo.getAttribute("frequency");
                    onDate = sdf.parse((String) messageWbo.getAttribute("onDate"));
                    uptoDate = sdf.parse((String) messageWbo.getAttribute("uptoDate"));
                    switch (frequency) {
                        case "once":
                            // do nothing
                            break;
                        case "daily":
                            ...
                            temp.set(Calendar.DATE, c.get(Calendar.DATE));
                            onDate = temp.getTime();
                            temp.add(Calendar.MINUTE, Integer.parseInt((String) messageWbo.getAttribute("period")));
                            uptoDate = temp.getTime();
                            break;
                        case "monthly":
                            temp.setTime(onDate);
                            temp.set(Calendar.YEAR, c.get(Calendar.YEAR));
                    ...
                    if (onDate.before(c.getTime()) && uptoDate.after(c.getTime())) {
                        messagesList.add(messageWbo);
                    }
