...
<HTML>
    ...
    <%
        ...
        List<WebBusinessObject> issuesVector = issueByComplaintUniqueMgr.getNonDistributedComplaints();
        UserMgr userMgr = UserMgr.getInstance();
        ArrayList<WebBusinessObject> usersList = userMgr.getAllActiveUsers();
        ...
        String sat, sun, mon, tue, wed, thu, fri, type, noResponse, ageComp, distributedTo, save;
    ...
    <CENTER>
        <FIELDSET class="set" style="width:96%;" >
            ...
            <form id="DISTRIBUTION_FORM" name="DISTRIBUTION_FORM" method="POST" action="<%=context%>/IssueServlet?op=distributeComplaints">
                <table style="margin-bottom: 20px; margin-top: 20px" ALIGN="center"  dir="<%=dir%>" border="0" width="40%" cellpadding="5" cellspacing="3" id="MainTable">
                    <tr onmouseover="this.className = 'rowHilight'" onmouseout="this.className = ''">
                        <td style="width: 200px;" class="silver_header td2 formInputTag boldFont"><%=distributedTo%>&nbsp;</td>
                        <td class="td2">
                            <select class="blackFont" id="employeeID" name="employeeID" style="font-size: large; width: 250px;">
                                <sw:WBOOptionList wboList="<%=usersList%>" displayAttribute = "fullName" valueAttribute="userId" scrollToValue="<%=employeeID%>"/>
                            </select>
                        </td>
                        <td class="td2" style="width: 100px;">
                            <input type="button" onclick="JavaScript: submitForm();" value="<%=save%>" class="button" style="width: 100px;"/>
                        </td>
                    </tr>
                </table>
                        ...
                        <TR>
                            <%
                                ...
                                if (wbo.getAttribute("ticketType").toString().equals("1")) {
                                    ...
                                    compType = "م. مالية";
                                    compStyle = "order";
                                } else if (wbo.getAttribute("ticketType").toString().equals("10")) {
                                    compType = "ت. جودة";
                                } else if (wbo.getAttribute("ticketType").toString().equals("25")) {
                                    compType = "طلب عميل";
                                    compStyle = "order";
                                }
                            %>
                            <TD>
                                <input type="checkbox" name="clientComplaintID" value="<%=wbo.getAttribute("compId")%>"/>
                            </TD>
                            <TD    style="cursor: pointer" onmouseover="this.className = '<%=compStyle%>'" onmouseout="this.className = ''">
                                <a href="#"> <font color="red"><%=wbo.getAttribute("businessID").toString()%></font><font color="blue">/<%=wbo.getAttribute("businessIDbyDate").toString()%></font>
                                </a>
                            </TD>
                            ...
                            <TD ><b><%=compType%></b></TD>
                            <TD ><b><%=wbo.getAttribute("subject") != null ? wbo.getAttribute("subject") : ""%></b></TD>
                            <TD ><b><%=wbo.getAttribute("businessCompId")%></b></TD>
                                    ...
                                    %>
                            <TD  ><b><%=complStatus != null ? complStatus : ""%></b></TD>
                            ...
                            <TD nowrap  ><font color="red"><%=sDay%> - </font><b><%=sDate + " " + sTime%></b></TD>
                                    <%}%>
                                    <% if (wbo.getAttribute("createdByName") != null && !wbo.getAttribute("createdByName").equals("")) {
                                            fullName = (String) wbo.getAttribute("createdByName");
                                        } else {
                                            fullName = "";
                                        }
                                    %>
