...
<html>
    ...
    <%
        ...
        year = request.getAttribute("year") != null ? (Integer) request.getAttribute("year") : periodicalCalendar.getTime().getYear() + 1900;
        periodicalCalendar.set(Calendar.MONTH, currentMonth);
        periodicalCalendar.set(Calendar.YEAR, year);
        ...
        /*if (startPeriodDay == 4 || (startPeriodDay == 5 )) {
            calRowsNumber = calRowsNumber + 1;
        }*/
        if(startPeriodDay!=6)
        {
            if(((6-startPeriodDay)+((calRowsNumber-1)*7)) < maxDate)
                calRowsNumber++;
        }
    ...
    <head>
        ...
        <script language="javascript" type="text/javascript">
            ...
            function changeMonth() {
                ...
                document.CALENDAR_FORM.action = "<%=context%>/CalendarServlet?op=showCalendar&month=" + month + "&year=" + year;
                document.CALENDAR_FORM.submit();
            }

            $(function() {
                centerDiv("show_appointment");
                $(".appPerDay").html("<img width='25px'height='25px' src='images/showcalendar.gif'/>")
            });
             function openAppointments(day){
                 var currentMonth=$("#currentMonth").val();
                 var currentYear = $("#currentYear").val();
                 
                 window.open('<%=context%>/CalendarServlet?op=getAppointmentsForUser&future=selectedDate&day='+day+'&currentMonth='+currentMonth+'&currentYear='+currentYear);
             }
        ...
        <style type="text/css">
            ...
            .save_app{
                ...
                cursor: pointer;
                margin-right:10px !important;float:right !important; 
            }
            .appPerDay{
    ...
    <FORM NAME="CALENDAR_FORM" METHOD="POST">
            <div style="width: 100%; text-align: center;">
                <!--a href="<%=context%>/CalendarServlet?op=getUsersCallsCalendar" title="مشاهدة تفاصيل المكالمات">
                    <img style="height: 35px; width: 35px; margin-right: 20px;" src="images/dialogs/phone.png"/>
                </a-->
                    
                <a href="<%=context%>/CalendarServlet?op=getUsersMeetingsCalendar" title="مشاهدة تفاصيل المقابلات">
                    <img style="height: 35px; width: 35px; margin-right: 20px;" src="images/dialogs/handshake.png"/>
                </a>
                    
                <!--a href="<%=context%>/CalendarServlet?op=getAppointmentsForUser&future=0" title="مشاهدة تفاصيل المكالمات و المقابلات">
                    <img style="height: 35px; width: 35px; margin-right: 20px;" src="images/user_appointments.png"/>
                </a-->
                <a href="<%=context%>/CalendarServlet?op=getAppointmentsForUser&future=yes" title= "مشاهدة تفاصيل المكالمات والمقابلات المستقبليه ">
                    <img style="height: 35px; width: 35px; margin-right: 20px;" src="images/future_appointment.png"/>
                </a>
                <a href="<%=context%>/CalendarServlet?op=getAppointmentsForUser&future=no" title= "مشاهدة تفاصيل المكالمات والمقابلات الماضيه ">
                    <img style="height: 35px; width: 35px; margin-right: 20px;" src="images/appointment_expired.png"/>
                </a>
                <a href="<%=context%>/CalendarServlet?op=getAppointmentsForUser&future=today" title= "مشاهدة تفاصيل مكالمات ومقابلات اليوم ">
                    <img style="height: 35px; width: 35px; margin-right: 20px;" src="images/future_appointment1.png"/>
                </a>
                ...
                <TABLE ALIGN="<%=align%>" WIDTH="80%" DIR="LTR" CELLPADDING="0" CELLSPACING="0" STYLE="border-width: 1px;border-color:white;background: rgba(255,255,255,1);
                    ...
                    <TR>
                        <%
                            ...
                            for (int i = 0; i < 8; i++) {
                                if (i < 7) {
                                    ...
                                    if (x >= currentDay) {
                        %>
                        <TD class="<%=dayBackColorStartWeek%>" STYLE="font-size:14px;border-width:1px;border-color:white;height:75px;" background="images/schedule.jpg">
                            ...
                            <div style="width: 100%;height: 55%;" id="div1">
                                <div  class="save_app" <% if (!cellValue.equals("0")) {%> onmouseover="this.style.cursor='pointer'" onclick="popupShowAppointment(<%=cellValue%>);"<% } %>  style="margin-left: auto;margin-right: auto;" ></div>
                                <div class="appPerDay" <% if (!cellValue.equals("0")) {%> onmouseover="this.style.cursor='pointer'" onclick="openAppointments('<%=cellValue%>');" <% } %> > 
                        ...
                        <TD class="<%=dayBackColorStartWeek%>"  STYLE="font-size:14px;border-width:1px;border-color:white;height:75px;" background="images/schedule.jpg">
                            ...
                            <div style="width: 100%;height: 55%;display: <%if (displayCountAppointment) { %>block <% } else { %>none<%}%>" id="div1">
                                <div  class="save_app" <% if (!cellValue.equals("0")) {%> onmouseover="this.style.cursor='pointer'" onclick="popupShowAppointment(<%=cellValue%>);"<% } %> style="margin-left: auto;margin-right: auto;" ></div>
                                 <div class="appPerDay" <% if (!cellValue.equals("0")) {%> onmouseover="this.style.cursor='pointer'" onclick="openAppointments('<%=cellValue%>');" <% } %> > 
                        ...
                        <TD STYLE="font-size:16;border-width:1px;border-color:white;" ROWSPAN="<%=calRowsNumber%>" BGCOLOR="#ddeeff">
                            <br /><br /><br /><br />
                            <select name="currentYear" id="currentYear" style="width: 130px;height: 35px;color: blue;font-weight: bold; font-size: 16px" onchange="JavaScript:changeMonth();">
                                <sw:OptionList optionList="<%=years%>" scrollTo="<%=Integer.toString(year)%>" />
                            </select>
                        </TD>
                                <%
                                        }
                    ...
                    <TR>
                        <%
                            for (int i = 0; i < 7; i++) {
                                ...
                                if (x >= currentDay) {%>
                        <TD class="<%=dayBackColorStartWeek%>" >
                            <div style="float: right; margin-top: 0px; width: 35px;height: 20px;border-radius: 3px 3px 3px 3px;text-align: center;display: <%if (displayCountAppointment) { %>block <% } else { %>none<%}%>" class="num" id="number"><%=numberAppointment%></div>
                            <div style="width: 100%;padding-top: 10px;height: 45%;">
                                <% String num = "";
                                    if (cellValue.equals("0")) {%>
                            ...
                            <div style="width: 100%;height: 55%;" id="div1">
                                <div class="save_app" onclick="popupShowAppointment(<%=cellValue%>);" style="margin-left: auto;margin-right: auto;"></div>
                                 <div class="appPerDay" <% if (!cellValue.equals("0")) {%> onmouseover="this.style.cursor='pointer'" onclick="openAppointments('<%=cellValue%>');" <% } %> > 
