...
<%
    ...
    String fieldNo   = "UNIT_NO";
    String colCode = "id", colName = "unitName", colNo = "unitNo";
    if (stat.equals("Ar")) {
        code = "&#1575;&#1604;&#1603;&#1608;&#1583;";  //
        name = "&#1575;&#1604;&#1573;&#1587;&#1605;";
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/ReportsServletThree?op=ListEquipments&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType +"&goToViewEquipment=ok";
                if(fieldName != null){
            ...
            function goToPage(pageIndex) {
                ...
                var fieldValue = getASSCIChar(getOperationOrFieldValue("fieldValue"));
                var operation  = getOperationOrFieldValue("operator");
                var url = "<%=context%>/ReportsServletThree?op=ListEquipments&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType +"&goToViewEquipment=ok";
                if(fieldName != null){
                    url += "&fieldName=" + fieldName;
                    if(fieldValue != null || fieldValue != ""){
            ...
            function sendInfo(unitNo,name,id) {
                var id=id;
                var unitNo = unitNo;
                var name = name;
                window.opener.document.<%=formName%>.searchInput.value = unitNo;
                window.opener.document.<%=formName%>.action = "<%=context%>/UnitDocReaderServlet?op=ViewImages&equipmentID="+id;
                        ...
                        <TABLE ID="tableSearch" class="blueBorder" ALIGN="center" DIR="<%=dir%>" CELLPADDING="0" CELLSPACING="0" width="90%">
                            <%
                                ...
                                if (conditions.size() > 0) {
                                    for (int j = 0; j < conditions.size(); j++) {
                                        ...
                                        if (field_name.equalsIgnoreCase(fieldCode)) {
                            ...
                            <TR>
                                <TD CLASS="" STYLE="text-align: <%=textAlign%>;font-size:15px;">
                                    <select id="fieldName">
                                        <option value ="<%=fieldNo%>" <%=selectedCode%>><%=code%></option>
                        ...
                        <table width="500">
                            ...
                            <%
                                ...
                                for (int i = 0; i < scheduleList.size(); i++) {
                                    ...
                                    String id = wbo.getAttribute(colCode).toString();
                                    if(id.equalsIgnoreCase((String)request.getAttribute(colCode))){
                                        continue;
                                    }
                                    String unitNo = wbo.getAttribute("unitNo").toString();
                                    String unitName = wbo.getAttribute(colName).toString();
                                    iTotal++;
                            ...
                            <%if (selectionType.equalsIgnoreCase("single")) {
                                    event = "onclick=\"return sendInfo('" + unitNo + "', '" + unitName + "', '" + id + "')\"";
                                }%>
                            <tr>
                                ...
                                <%if (selectionType.equalsIgnoreCase("multi")) {%>
                                <td class="thTitle">
                                    <input type="checkbox" name="relatedunitNos" value="<%=unitNo%>"/>
                                </td>
                                <%}%>
                                <td class="td2" style="width: 20px;">
                                    <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"> <%=unitNo%></font> </b>
                                        <input type="hidden" name="id" value="<%=unitNo%>"/>
                                    </a>
                                </td>
                                <td>
                                    <a  href="#" <%=event%>>
