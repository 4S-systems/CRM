    ...
    $(document).ready(function(){
        ...
        function checkAllProjects() {
           ...
           alert("projectArr.length : ");
            ...
            var projectArr = document.getElementsByName('<%=fieldName%>');
           alert(projectArr.length);
        ...
        function getEquipmentInPopup(){
            ...
            var sites = document.getElementsByName('<%=fieldName%>');
            var count = 0;
        ...
        <TABLE  WIDTH="200"  CELLPADDING="0" CELLSPACING="0" STYLE="border:0px; width:100%;" ALIGN="<%=align%>" DIR="<%=dir%>">
            ...
            <TR>
                <TD CLASS="mainHeaderNormal" WIDTH="100%" STYLE="<%=style%>;padding-<%=align%>:5;">
                    <INPUT TYPE="CHECKBOX" NAME="allProjectsCbx" ID="allProjectsCbx" />
                    <%=checkAllStr%>
                </TD>
            </TR>
            ...
            <TR>
                <TD CLASS="act_sub_heading" WIDTH="100%" STYLE="<%=style%>;padding-<%=align%>:5;">
                    <INPUT TYPE="CHECKBOX" NAME="<%=fieldName%>" ID="mainProject<%=i%>" value="<%=mainProjectId%>" <%=check%> onclick="checkSubProjects(<%=i%>);getChecked()"/>
                    <input type="hidden" name="projectName" value="<%=mainProjectWbo.getAttribute("projectName")%>" />
            ...
            <TR>
                <TD WIDTH="100%" STYLE="<%=style%>;padding-<%=align%>:25;background: white;">
                    <INPUT TYPE="CHECKBOX" NAME="<%=fieldName%>" ID="subProject<%=i%><%=j%>" value="<%=subProjectId%>" <%=check%> onclick="getChecked()"/>
                    <input type="hidden" name="projectName" value="<%=subProjectWbo.getAttribute("projectName")%>" />
