...
<%
    ...
    String code,eqCode, name, all, list_title_page, listLabel, dir, fieldValue, defaultView, cancel, msg,
    ...
    List<WebBusinessObject> scheduleList = (List<WebBusinessObject>) request.getAttribute("scheduleList");
    ...
    /*String newRelation = (String)request.getAttribute("newRelation");
    relations.append("<select STYLE='padding:10px; font-size: 12px; font-weight: bold;' name='relationTypes'>");

    String relation = "";

    if(relation != null && relation.equals("")){
        relations.append("<option selected STYLE='padding:5px; font-size: 14px;' CLASS='blueBorder blueBodyTD' value='" + newRelation + "' >" + newRelation + "</option>");
    }
    for (WebBusinessObject scheduleRelationTypesWbo : scheduleRelationTypes) {
        relation = (String) scheduleRelationTypesWbo.getAttribute("relationType");
        if(relation.equalsIgnoreCase(newRelation)){
           continue;
        }
        relations.append("<option STYLE='padding:5px; font-size: 14px;' CLASS='blueBorder blueBodyTD' value='" + relation + "' >" + relation + "</option>");                            
    }
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function submitForm(url){
                ...
                document.LIST_SCHEDULES_FORM.action = url;
                document.LIST_SCHEDULES_FORM.submit();
            }
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/ReportsServletThree?op=ListEquipments&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                if(fieldName != null){
            ...
            function goToPage(pageIndex) {
                ...
                var fieldValue = getASSCIChar(getOperationOrFieldValue("fieldValue"));
                var operation  = getOperationOrFieldValue("operator");
                var url = "<%=context%>/ReportsServletThree?op=ListEquipments&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                if(fieldName != null){
                    url += "&fieldName=" + fieldName;
                    if(fieldValue != null || fieldValue != ""){
            ...
            function sendInfo(){
                var row;
                var insertedTable = window.opener.document.getElementById('Table1');
                var check = document.getElementsByName('relatedunitNos');
                ...
                for(var i = 0; i < length; i++){
                    if(check[i].checked == true){
                        ...
                        //alert("lastrow : " + lastrow + "\n table rows number : " + (insertedTable.rows.length - 1)+ "\n index i : " + i);
                        insertCells(row, id[i].value, unitName[i].value, i + lastrow);
                        count++;
                    }
                }
                if(count > 0){
            ...
            function insertCells(row, id, unitName, index){
                ...
                //cell0.innerHTML = "<input type=\"hidden\" name=\"relatedunitNos\" value="+ id +" />";
                
                cell0.style.borderWidth = "1px";
                cell0.style.borderColor = "#000080";
                cell0.style.textAlign = "left";
                cell0.style.fontWeight = "bold";
                cell0.style.color = "black";
                cell0.style.padding = "10px";
                cell0.innerHTML = index + "<input type=\"hidden\" name=\"relatedunitNos\" value="+ id +" />";
                ...
                cell1.style.borderColor = "#000080";
                cell1.style.fontWeight = "bold";
                ...
                cell2.style.borderColor = "#000080";
                cell2.style.fontWeight = "bold";
                cell2.style.textAlign = "center";
                
                cell2.innerHTML = "<%=relations%>";

                cell3.style.borderWidth = "1px";
                cell3.style.borderColor = "#000080";
                cell3.style.textAlign = "center";
                cell3.style.fontWeight = "bold";
                cell3.style.color = "black";
                cell3.style.padding = "10px";
                cell3.innerHTML = '---';// '<input type="checkbox" name="deleterow" value="" onclick="Delete()" />';
            ...
            function sendEquipmentInfo(id, name){
                var id=id;
                var name = name;
    ...
    <body>
        <div class="content" align="center" dir="<%=dir%>" style="position:relative; overflow:auto;">
            <form method="post" name="LIST_SCHEDULES_FORM" action="">
                ...
                <fieldset class="border" style="width: auto;">
                    ...
                    <div style="padding: 5px; background: #d3d5d4;">
                            ...
                            <%
                                ...
                                if (conditions.size() > 0) {
                                    for (int j = 0; j < conditions.size(); j++) {
                                        ...
                                        if (field_name.equalsIgnoreCase("UNIT_NO")) {
                                            selectedCode = "checked";
                                        } else if (field_name.equalsIgnoreCase("UNIT_NAME")) {
                        ...
                        <input type="radio" <%=selectedName%> id="fieldName" name="fieldName" value="UNIT_NAME"><b><%=name%></b>
                        
                    </TD>
                                    
                                
                                <TD CLASS="" STYLE="text-align: <%=textAlign%>;" id="CellData" colspan="2" rowspan="2">
                            ...
                            <TR>
                    <TD CLASS="blueBorder blueHeaderTD" STYLE="text-align:center;font-size:15px;">
                        <input type="radio" <%=selectedCode%>  name="fieldName" value="UNIT_NO"><b><%=eqCode%></b>
                    </TD>
                </TR>
                        ...
                        <table width="500">
                            ...
                            <%
                                ...
                                for (int i = 0; i < scheduleList.size(); i++) {
                                    ...
                                    wbo = (WebBusinessObject) scheduleList.get(i);
                                    String id = wbo.getAttribute("id").toString();
                                    if(id.equalsIgnoreCase((String)request.getAttribute("id"))){
                                        continue;
                                    }
                                    String unitName = wbo.getAttribute("unitName").toString();
                                    String unitNo = wbo.getAttribute("unitNo").toString();
                                    iTotal++;
                            ...
                            <%if (selectionType.equalsIgnoreCase("single")) {
                                    event = "onclick=\"return sendEquipmentInfo('" + id + "', '" + unitName + "')\"";
                                }%>
                            <tr>
                                ...
                                <%if (selectionType.equalsIgnoreCase("multi")) {%>
                                <td class="thTitle">
                                    <input type="checkbox" name="relatedunitNos" value="<%=wbo.getAttribute("id")%>"/>
                                </td>
                                <%}%>
                                <%--<td class="td2" style="width: 20px;">
                                    <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"> <%=id%></font> </b>
                                        <input type="hidden" name="id" value="<%=wbo.getAttribute("id")%>"/>
                                    </a>
                                </td>--%>
                                <td>
                                    <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"> <%=unitName%> </font> </b>
                                        <input type="hidden" name="unitName" value="<%=wbo.getAttribute("unitName")%>"/>
                                        <input type="hidden" name="id" value="<%=wbo.getAttribute("id")%>"/>
                                    </a>
                                </td>
                                <td>
                                   <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"> <%=unitNo%> </font> </b>
                                        <input type="hidden" name="unitNo" value="<%=wbo.getAttribute("unitNo")%>"/>
                                        <input type="hidden" name="id" value="<%=wbo.getAttribute("id")%>"/>
                                        </a>
