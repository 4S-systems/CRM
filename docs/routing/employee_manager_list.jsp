...
<%
    ...
    String code, name, all, fullNameStr, list_title_page, listLabel, dir, fieldValue, defaultView, cancel, msg,
    ...
    String managerID = request.getAttribute("managerID").toString();
    ...
    String sResponsible, sInfo, sAssist, searchForName;
    if (stat.equals("Ar")) {
        code = "&#1575;&#1604;&#1603;&#1608;&#1583;";  //
        name = "اسم الموظف";
        listLabel = "قائمة الموظفين";
        list_title_page = "قائمة الموظفين";
        searchForName = "بحث عن موظف";
        ...
        fullNameStr = "Full Name";
        sResponsible = "For responsibility";
        sInfo = "For Info";
        sAssist = "For assist";
    }
    ...
    List<WebBusinessObject> listUnattachedEmployees = (List<WebBusinessObject>) request.getAttribute("usersList");
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/ComplaintEmployeeServlet?op=usersUnderManager&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType + "&managerID=<%=managerID%>";
                if (fieldValue != null) {
            ...
            function goToPage(pageIndex) {
                ...
                var url = "<%=context%>/ComplaintEmployeeServlet?op=usersUnderManager&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType + "&managerID=<%=managerID%>";
                submitForm(url);
            }
            ...
            function send_Info() {
                ...
                var showDiv = window.opener.document.getElementById('showDiv');

                var statusCode = window.opener.document.getElementById('status_code').value;
                ...
                var fullNameStr = document.getElementsByName('<%=fieldName%>');
                var commentsArr = document.getElementsByName('comments');
                var responsibleArr = document.getElementsByName('responsible');
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    if (check[i].checked == true) {
                        ...
                        if (!opener.window.users[mgrId[i].value]) {
                            ...
                            insert_Cells(row, lastrow, count + lastrow, fullNameStr[i].value, mgrId[i].value, commentsArr[i].value, responsibleArr[i], statusCode);
            ...
            function insert_Cells(row, order, index, fullNameStr, userId, comments, responsible, statusCode) {
                ...
                cell0.innerHTML = '<input type="hidden" name="index" id="index" size="10" value="' + index + '" /><span id="order" name="order" value="' + index + '">' + index + "</span><input type=\"hidden\" id=\"empId\" name=\"empId\" value=" + userId + " />";
                ...
                cell2.innerHTML = fullNameStr +
                    '<input type="hidden" name="userName" id="userName' + order + '" value="' + fullNameStr + '" />';

                cell3.className = classname;
                cell3.innerHTML = '<input type="text" name="comments" id="comments" size="10" value="" />';

                //                cell4.className = classname;
                //          
                //                if (statusCode == "2") {
                //                    var x = '<select name="responsible" id="responsible"><option value="1"><%=sResponsible%></option><option value="2"><%=sAssist%></option><option value="3"><%=sInfo%></option></select>';
                //                    cell4.innerHTML = x;
                //                } else {
                //                    var x = '<select name="responsible" id="responsible"><option value="2"><%=sAssist%></option><option value="3"><%=sInfo%></option></select>'
                //                    cell4.innerHTML = x;
                //                }

                cell4.className = classname;
                cell4.innerHTML = '<div id="save' + index + '" class="save__" style="background-position: bottom;" onclick="saveComplaintEmployee(this, ' + index + ')"></div>';

                cell5.className = classname;
                cell5.innerHTML = '<div class="remove" onclick="remove(this, ' + order + ')" id="' + userId + '" title="' + userId + '"  style="margin-right:auto;margin-left:auto;"></div>';
        ...
        <div class="content" align="center" dir="<%=dir%>" style="position:relative; overflow:auto;">
            <form method="post" name="LIST_USERS_FORM" action="">
                <DIV align="center" STYLE="color:blue;">
                    ...
                    <button  type=button STYLE=" " ONMOUSEDOWN="send_Info();"  style=" border:none;background-color: transparent;
                             " ><IMG VALIGN="BOTTOM"   SRC="images/user-check-icon.png" style=" border:none;background-color: transparent;"></button>

                    <%}%>
                    <button  onclick="JavaScript: closeForm();" class="button"><IMG  style=" border:none;background-color: transparent;" VALIGN="BOTTOM"   SRC="images/application-delete-icon.png"> </button>
                        ...
                        <TABLE ID="tableSearch" class="blueBorder" ALIGN="center" DIR="<%=dir%>" CELLPADDING="0" CELLSPACING="0" width="90%">
                            ...
                            <TR>
                                <TD CLASS="" STYLE="text-align: <%=textAlign%>;font-size:15px;">
                                    <input type="hidden" name="fieldName" id="fieldName" value="<%=field_name%>" /><b><%=searchForName%> :</b>
                                </TD>
                        ...
                        <table width="500">
                            ...
                            <tr>
                                ...
                                <th class="thTitle" style="width: 20px;">
                                    <!--<input type="radio" id="checkAll" onclick="CheckAll(this.id, 'relatedEmpsIds');" value=""/>-->
                                </th>
                                <%}%>
                                <!--                                <th class="thTitle">
                                <%=code%>
                            </th>-->
                                <th class="thTitle" style="width: 100px;">
                                    <%=name%>
                                </th>
                            ...
                            <%
                                ...
                                String comments = "", responsible = "";

                                for (int i = 0; i < listUnattachedEmployees.size(); i++) {

                                    wbo = (WebBusinessObject) listUnattachedEmployees.get(i);
...
//                                    codeValue = wbo.getAttribute(fieldCode).toString();
                                    ...
                                    responsible = (String) wbo.getAttribute("responsible");
                                    iTotal++;
                            ...
                            <tr>
                                ...
                                <%if (selectionType.equalsIgnoreCase("multi")) {%>
                                <td class="thTitle">
                                    <input type="radio" name="relatedEmpsIds" value="<%=idValue%>"/>
                                </td>
                                <%}%>
                                <!--                                <td class="td2" style="width: 20px;">
                                                                    <a  href="#" <%=event%>>
                                ...
                                <td>
                                    <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"><%=fullName%></font></b>                                        
                                    </a>
                                </td>
                            </tr>
