...
<%
    ...
    String sResponsible, sInfo, sAssist;
    if (stat.equals("Ar")) {
        code = "&#1575;&#1604;&#1603;&#1608;&#1583;";  //
        name = "اسم النظام";
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/ComplaintEmployeeServlet?op=listUnattachedEmployees&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType + "&complaintId=<%=complaintId%>";
                if (fieldValue != null) {
...
//                $.ajax({
//                        type: "get",
//                        url: "<%=context%>/ComplaintEmployeeServlet?op=getEmpByGroup2&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType + "&complaintId=<%=complaintId%>",
//                        data: {count: current},
...
//                    });
//               
                var url = "<%=context%>/ComplaintEmployeeServlet?op=getEmpByGroup2&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType + "&complaintId=<%=complaintId%>";
                submitForm(url);
            }
            ...
            function send_Info() {
                ...
                var selectedId = window.opener.document.getElementById("ids").value;
//                alert(selectedId);

                insertedTable.style.display = "block";
//                insertedTable.className = "blueBorder";
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    if (check[i].checked == true) {
                        ...
                        if (!opener.window.users[mgrId[i].value]) {
                            ...
                            insert_Cells(row, lastrow, count + lastrow, fullNameStr[i].value, mgrId[i].value, commentsArr[i].value, responsibleArr[i], selectedId);
...
//               window.opener.document.getElementById('tblDataDiv').bPopup();
//                window.opener.document.getElementById('tblDataDiv').css("display", "block");
...
//                     window.opener.test();
            ...
            function insert_Cells(row, order, index, fullNameStr, userId, comments, responsible, selectedId) {
                ...
                cell0.innerHTML = '<input type="hidden" name="index" id="index" size="10" value="' + index + '" /><span id="order" name="order" value="' + index + '">' + index + "</span><input type=\"hidden\" id=\"empId\" name=\"empId\" value=" + userId + " />" + "<input type=\"hidden\" id=\"selectedId\" value=" + selectedId + "/>";
                ...
                cell4.innerHTML = '<select name="responsible" id="responsible"><option value="1"><%=sResponsible%></option><option value="2"><%=sAssist%></option><option value="3"><%=sInfo%></option></select>';

                cell5.className = classname;
                cell5.innerHTML = '<div id="save' + index + '" class="save" style="background-position: bottom;" onclick="saveComplaintEmp(this, ' + index + ')"></div>';

                cell6.className = classname;
                cell6.innerHTML = '<span class="remove" onclick="remove(this, ' + order + ')" id="' + userId + '" title="' + userId + '"  ></span>';
