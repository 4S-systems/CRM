...
<%
    ...
    String sawper,complaintName,responsibility;
    ...
    if (stat.equals("Ar")) {
        ...
        complaintName="الإداره";
        responsibility="المسئوليه";
    } else {
    ...
    List<WebBusinessObject> listUnattachedEmployees = (List<WebBusinessObject>) request.getAttribute("compEmps");
    WebBusinessObject wbo = null;
    String fieldId = "userId";
    String fieldName = "fullName";
//    String fieldCode = "sodicId";

    String tableNameField = "FULL_NAME", tableIdField = "USER_ID";//, tableCodeField = "SODIC_ID";
%>
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/ComplaintEmployeeServlet?op=listResEmps&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                if(fieldName != null){
            ...
            function goToPage(pageIndex) {
                ...
                var url = "<%=context%>/ComplaintEmployeeServlet?op=listResEmps&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                submitForm(url);
            }
            ...
            function send_Info(){
               ...
               // alert('length='+length)
                var empId = document.getElementsByName('<%=fieldId%>');
               // alert('values: empId= '+empId.value);
//                var sodicId = document.getElementsByName('<//%=fieldCode%>');
                var userName = document.getElementsByName('<%=fieldName%>');
                //alert('values: userName= '+userName.value);
//                var commentsArr = document.getElementsByName('empId');
...
//                    alert('values: empId= '+empId[i].value+' &userName= '+userName[i].value);
                    if(check[i].checked == true){
//                        alert('test');
                        if(!opener.window.users[empId[i].value]){
//                            alert('test2');
                           ++insertCount;
                            row = insertedTable.insertRow(insertCount);   
//                    alert('count= '+count);
                            insert_Cells(row, lastrow, count + lastrow, empId[i].value,userName[i].value);
...
//                     alert('test');
            ...
            function insert_Cells(row, order,index,empId,userName){
//        alert('test');
...
//                var cell4 = row.insertCell(4);
                ...
                cell0.innerHTML = '<input type="hidden" name="index" id="index" size="10" value="'+index+'" /><span id="order" name="order" value="'+index+'">' +index+ "</span>";
                               
                cell1.className = classname;
                cell1.innerHTML = userName+
                    '<input type="hidden" name="fullName" id="fullName'+order+'" value="'+userName+'" /><input type="hidden" name="userId" id="userId'+order+'" value="'+empId+'" />';
                
//                cell3.className = classname;
//                cell3.innerHTML = 
//                    '<input type="hidden" name="userId" id="userId'+order+'" value="'+empId+'" />';
                                          
                cell2.className = classname;
                cell2.innerHTML = '<div id="save'+index+'" class="save" style="background-position: bottom;" onclick="addEmpsToGroup(this, '+index+')"></div>';
                opener.window.users[empId]=1;
                    ...
                    <div>
                        <table width="500">
                            ...
                            <%
                                ...
                                String event = "single";
                                String idValue, nameValue,compName,resp="";
                                ...
                                for (int i = 0; i < listUnattachedEmployees.size(); i++) {
                                    ...
                                    compName = wbo.getAttribute("complaintName").toString();
                                    if(wbo.getAttribute("responsiblity").toString().equals("FOR INFO")){
                                        resp="للعلم";
                                    }else if(wbo.getAttribute("responsiblity").toString().equals("FOR HELP")){
                                        resp="للمساعده";
                                    }
...
<!--                                <td class="td2" style="width: 20px;">
                                ...
                                <td>
                                    <b style="text-decoration: none"><font color="black" size="3"> <%=compName%> </font> </b>
                                </td> 
                                
                                <td>
                                    <b style="text-decoration: none"><font color="black" size="3"> <%=resp%> </font> </b>
                                </td> 
                            </tr>
