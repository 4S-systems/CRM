...
<%
    ...
    String Dstatus = (String)request.getAttribute("Dstatus");
    ...
    String dd = (String) userWbo1.getAttribute("groupID");
    groupPrev = groupPrevMgr.getOnArbitraryKey(userWbo1.getAttribute("groupID").toString(), "key1");
    ...
    String beDate = request.getAttribute("beginDate") != null ? (String) request.getAttribute("beginDate") : "";
    String eDate = request.getAttribute("endDate") != null ? (String) request.getAttribute("endDate") : "";
    
    String stat = (String) request.getSession().getAttribute("currentMode");
    String title, fDateStr, tDateStr, srch, clntNm, clntNo, clntMb, clntPh, clntIntPh, dstTo, wdWth, wdTime, rqTyp, dstMsg, clientClassification, dir;
    if(stat.equals("En")){
...
<html>
	...
	<script>
		...
		$("#employeeId").select2();
                ...
                var Dstatus = '<%=Dstatus%>';
                if(Dstatus != null && Dstatus =="true"){ 
                    alert("All Future Appointments Has Been Deleted");
                } else if(Dstatus != null && Dstatus =="false") {
                    alert("Future Appointments not Deleted");
                }
	    });
	    
	    
	    function getWithDraws(){
		document.withdraw_report_form.action = "<%=context%>/IssueServlet?op=withdrawReport&my=" + <%=my%>;
	    ...
	    function distribution(mode) {
		    ...
		    $("#employeeId").focus();
                    ...
                    document.withdraw_report_form.action = "<%=context%>/AutoPilotModeServlet?op=distributeWithdrawCustomers&mode=" + mode + "&fromURL=unHandledClients" + "&loggedOnly=" + loggedOnly + "&requestType=" + $("#requestType").val();
                    document.withdraw_report_form.submit();
                }
            }
            
            function deleteAllFutureApp(){
                if ($("input[name=customerId]:checked").length <= 0) {
                    alert(" Choose At Least One Client ");
                } else if ($("#employeeId option:selected").length <= 0) {
                    alert(" Please Select Employee ");
                    $("#employeeId").focus();
                } else if ($("#employeeId option:selected").length > 1) {
                    alert(" You Can Select Only One Employee ");
                    $("#employeeId").focus();
                } else {
                    $("#DAllFutApp").attr("disabled", "true");
                    document.withdraw_report_form.action = "<%=context%>/IssueServlet?op=deleteAllFutureAppointments";
                    document.withdraw_report_form.submit();
                }
            }
	</script>
	...
	<fieldset class="set" style="width: 80%;">
	    ...
	    <form id="withdraw_report_form" name="withdraw_report_form" method="post">
                <table style="width: 60%; padding-top: 2%; margin-top: 2%;" dir="<%=dir%>">
		    <tr>
                ...
                <table ALIGN="center" DIR="<%=dir%>" bgcolor="#dedede" WIDTH="60%" style="border: none; padding-bottom: 2%; margin-bottom: 2%;" CELLSPACING=2 CELLPADDING=1>
                    <tr>
                        <td bgcolor="#F7F6F6" style="text-align:center; height: 100%; padding: 10px;  border: none;" WIDTH="33%">
                    ...
                    <%if (userPrevList.contains("DELETE_APPOINTMENT")) {%>
                        <tr>
                            <td bgcolor="#F7F6F6" style="text-align:center; height: 100%; padding: 10px;  border: none;" WIDTH="33%">
                                <button id="DAllFutApp" type="button" onclick="JavaScript: deleteAllFutureApp();" value="" style="margin-left: 5%; text-align: center; width: 150px; font-size: 16px; color: blue; font-weight: bold;">
		...
		<div style="width: 95%;  padding-bottom: 2%; padding-top: 2%; margin-bottom: 2%; margin-top: 2%;">
                    <table id="withdrawTbl" style="width: 100%;" dir="<%=dir%>">
			<thead>
			    <tr>
				<th style="width: 1%;">
			...
			<tbody>
				...
				<tr>
				    ...
				    <td>
					 <%=withdrawWbo.getAttribute("rateName") != null ? withdrawWbo.getAttribute("rateName") : ""%> 
				    </td>
