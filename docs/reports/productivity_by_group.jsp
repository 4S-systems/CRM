...
<html>
    <%
        ...
        List<WebBusinessObject> productivityList = (List) request.getAttribute("productivityList");
        ...
        if (stat.equals("En")) {
            ...
            title = "Productivity by Group";
            print = "get report";
        } else {
            align = "center";
            dir = "RTL";
            style = "text-align:Right";
            title = "اﻷنتاجية بالمجموعة";
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                chart = new Highcharts.Chart({
                    ...
                    title: {
                        text: 'أنتاجية مجموعة "' + $("#groupId option:selected").text() + '" في الربع ' + $("#quarter option:selected").text() + ' لعام ' + $("#year option:selected").text()
                    },
                    subtitle: {
                        text: ''
                    },
                    xAxis: {
                        ...
                        categories: [<% if (productivityList != null) {
                                for (int i = 0; i < productivityList.size(); i++) {
                                    WebBusinessObject wbo_ = productivityList.get(i);
                                    if (i > 0) {
                                        out.write(",");
                                    }
            ...
            {
                var id = $("#groupId").val();
                document.ProductivityByGroup.action = "<%=context%>/ReportsServletThree?op=getProductivityByGroup&id=" + id;
                document.ProductivityByGroup.submit();
            }
    ...
    <body>
        <form name="ProductivityByGroup" method="post">
            <fieldset class="set" style="width:85%;border-color: #006699">
                ...
                <br>
                <% if (productivityList != null && productivityList.size() > 0) {%>
                ...
                <div style="width: 85%;margin-right: auto;margin-left: auto;" id="showClients">
                    <table ALIGN="<%=align%>" dir="<%=dir%>" WIDTH="100%" id="clients" style="">
                        ...
                        <tbody>
                            <%
                                int total = 0;
                                for (WebBusinessObject wbo_ : productivityList) {
                                    try {
                            ...
                            <tr style="cursor: pointer" id="row">
                                ...
                                <td>
                                    ...
                                    <b style="cursor: hand;" onclick="JavaScript: openInNewWindow('<%=context%>/ReportsServletThree?op=getProductivityClientsReport&groupID=<%=request.getParameter("groupId")%>&fromDate=<%=wbo_.getAttribute("startDate")%>&toDate=<%=wbo_.getAttribute("endDate")%>')"><%=wbo_.getAttribute("total")%></b>
                                    <%} else {%>
                                    <b>0</b>
                                    <%}%>
                                </td>
                        ...
                        <tfoot>
                            <tr>
                                <th style="color: #005599 !important;font-size: 14px; font-weight: bold;" colspan="2">أجمالي العملاء</th>
