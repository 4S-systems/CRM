...
<%@ page import="com.silkworm.common.TimeServices,com.silkworm.common.*"%>
<%@taglib prefix="sw" uri="/WEB-INF/swtaglib.tld" %>
...
<HTML>
    <%
        ...
        String[] projectWorkerAtt = appCons.getprojectWorkerAtt();
        String[] projectWorkerTitle = appCons.getprojectWorkerHeaders();

        int s = projectWorkerAtt.length;
        ...
        WebBusinessObject worker = userMgr.getOnSingleKey(WorkerID);
        workername = (String) worker.getAttribute("userName");
    ...
    <BODY>
        ...
        <TABLE WIDTH="600" CELLPADDING="0" CELLSPACING="0" STYLE="border-right-WIDTH:1px;">
            <TR VALIGN="MIDDLE">
                <TD COLSPAN="2" CLASS="tabletitle"  STYLE="border-left-WIDTH: 1;text-align:left;">
                    <font size="5"> <b>Detailed Worker Schedule in Site '<%=projectName%>'</b></font>
                </TD>
                <TD CLASS="tabletitle" STYLE="">
                    <IMG WIDTH="20" HEIGHT="20" SRC="images/corner.gif">
                </TD>
            </TR>
            <TR style="display: <%=userPrevList.contains("EXCEL") ? "" : "none"%>;">
                <TD COLSPAN="3">
                    <A HREF="<%=context%>/SearchServlet?op=HoursWorkerExcel&projectName=<%=request.getParameter("projectName")%>&workerName=<%=workername%>">
        ...
        <BR>
        <TABLE WIDTH="700" CELLPADDING="1" CELLSPACING="0" STYLE="border-right-WIDTH:1px;" >
                ...
                <% for(int i = 0;i<s;i++)
    {
                %>
                <TD>
                    <p ><b><font size="4" color="#00ff00">
                        <%
                            if (i <= 3) {

                        %>
                        <a  href="" onclick="return sortTable2(<%=i%>)" style="color:#0000FF;"><B><%=projectWorkerTitle[i]%></B></a>
               
                        <% 
                            } else { %>
                        <%=projectWorkerTitle[i]%>
                        <%
                            }
            ...
            <tbody id="planetData2">
                ...
                iTemp = new Double((String) wbo.getAttribute("actualFinishTime"));
                hTotal = hTotal + iTemp.intValue();
                    ...
                    <% for(int i = 0;i<s;i++)
    {
                ...
                %>
                <p dir="ltr"><b><font size="4" color="#800080">Worker Name : <%=workername%></font></b></p>
                <p dir="ltr"><b><font size="4" color="#800080">Total Hours : <%=hTotal%></font></b></p>
            </tbody>
            
            <TR>
                <TD bgcolor="#DBA700" CELLPADDING="30" COLSPAN="2">
                    <p ><b><font size="3" color="#0000FF">
                        <B>Total Hours</B>
                    </FONT></b></p>
                </TD>
                <TD bgcolor="#DBA700" CELLPADDING="8">
                    <p><font size="3" color="#0000FF">
                        <B><%=hTotal%></B> 
                    </font></b></p>
