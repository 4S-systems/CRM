...
<html>
    <%
        ...
        ArrayList<WebBusinessObject> meetingResult = (ArrayList) request.getAttribute("data");
    ...
    <body>
        ...
        <div style="width: 100%; text-align: center; direction: rtl">
            <font size="3">مقبلات الموظف : </font><b><font size="3" color="red"><%=userName%></font></b>
            <br>
            <br>
        ...
        <% if (meetingResult != null && !meetingResult.isEmpty()) {%>
        <div style="width: 90%">
            <table class="display" id="indextable" align="center" dir="<%=dir%>" width="100%" cellpadding="0" cellspacing="0">
                <thead>
                    <tr>
                        ...
                        <th CLASS="blueHeaderTD backgroundTable"style="width: 11%; border-width: 0px ;text-align: center">تاريخ المقابلة</th>
                        <th CLASS="blueHeaderTD backgroundTable"style="width: 10%; border-width: 0px ;text-align: center">نتيجة المقابلة</th>
                        ...
                        <th CLASS="blueHeaderTD backgroundTable"style="width: 37%; border-width: 0px ;text-align: center">نوع المكالمة</th>
                    </tr>
                </thead> 

                <tbody  id="planetData2">  
                    <%
                        for (WebBusinessObject wbo : meetingResult) {
                            iTotal++;

                            String color;

                            if (wbo.getAttribute("current_status").toString().equals("24")) {
                                color = "yellow";
                            } else {
                                color = "red";
                            }
                    %>
                    <tr style="padding: 1px; background-color: color">
                        <td style="border-width: 1px ; text-align: <%=color%>">
                            <%=iTotal%>
                        </td>
                        ...
                        <td style="border-width: 1px ; text-align: center; background-color: <%=color%>">
                            <%if (wbo.getAttribute("current_status").toString().equals("26")) {%>
                            ناجحة
                            <%} else if (wbo.getAttribute("current_status").toString().equals("24")) {%>
                            معتني بها
                            <%} else {%>
                            غير معتني بها
                            <%}%>
