...
String dir=null;
String style=null;
String lang,langCode,tit,save,cancel,TT,SNA,tit1,RU,EMP,STAT, Fstatues,Sstatues,NO;
String message=null;
if(stat.equals("En")){
    ...
    Sstatues="File attaching successfully";
    Fstatues="File attaching field";
    ...
    EMP="&#1573;&#1587;&#1605; &#1575;&#1604;&#1608;&#1592;&#1601;";
     STAT=" &#1581;&#1575;&#1604;&#1577; &#1575;&#1604;&#1573;&#1585;&#1601;&#1575;&#1602;";
   Sstatues = "&#1578;&#1605; &#1575;&#1604;&#1575;&#1585;&#1601;&#1575;&#1602;  &#1576;&#1606;&#1580;&#1575;&#1581;";
    Fstatues ="&#1604;&#1605; &#1610;&#1578;&#1605; &#1575;&#1604;&#1575;&#1585;&#1601;&#1575;&#1602;";
...
<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
 ...
 function cancelForm()
        ...
        if(source=="mainCat"){
            document.IMG_FORM.action = "<%=context%>/ScheduleServlet?op=ViewSchedule&periodicID=<%=scheduleID%>&mainCatId=<%=schWbo.getAttribute("mainCatTypeId")%>&source=<%=source%><%=sType%>";
        }else if(source=="Cat"){
            document.IMG_FORM.action = "<%=context%>/ScheduleServlet?op=ViewSchedule&periodicID=<%=scheduleID%>&equipmentCat=<%=schWbo.getAttribute("equipmentCatID")%>&source=<%=source%><%=sType%>";
        } else {
            document.IMG_FORM.action = "<%=context%>/ScheduleServlet?op=ViewSchedule&periodicID=<%=scheduleID%>&equipmentID=<%=schWbo.getAttribute("equipmentID")%>&source=<%=source%>";
        }
        ...
        if(source=="mainCat"){
            document.IMG_FORM.action = "<%=context%>/ScheduleDocWriterServlet?op=AttachImage&fileExtension=<%=fileExtension%>&scheduleID=<%=scheduleID%>&docType=<%=docType%>&mainCatId=<%=schWbo.getAttribute("mainCatTypeId")%>&source=<%=source%><%=sType%>";
        }else if(source=="Cat"){
            document.IMG_FORM.action = "<%=context%>/ScheduleDocWriterServlet?op=AttachImage&fileExtension=<%=fileExtension%>&scheduleID=<%=scheduleID%>&docType=<%=docType%>&equipmentCat=<%=schWbo.getAttribute("equipmentCatID")%>&source=<%=source%><%=sType%>";
        } else {
            document.IMG_FORM.action = "<%=context%>/ScheduleDocWriterServlet?op=AttachImage&fileExtension=<%=fileExtension%>&scheduleID=<%=scheduleID%>&docType=<%=docType%>&equipmentID=<%=schWbo.getAttribute("equipmentID")%>&source=<%=source%>";
        }
    ...
    if(imAttStatus.equalsIgnoreCase("ok")){
       message  = Sstatues;
           ...
           message =Fstatues;
                ...
                %>
        <tr><td class="td" align=<%=align%>>  <font color="red" size="4"> <%= message%>   </td>
