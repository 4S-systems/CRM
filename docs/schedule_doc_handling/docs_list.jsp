...
function normalizeString(s) {
 ...
 function cancelForm(url)
        ...
        if(source=="mainCat"){
            url="<%=context%>/ScheduleServlet?op=ViewSchedule&periodicID=<%=scheduleID%>&mainCatId=<%=schWbo.getAttribute("mainCatTypeId")%>&source=<%=source%><%=sType%>";
        }else if(source=="Cat"){
            url="<%=context%>/ScheduleServlet?op=ViewSchedule&periodicID=<%=scheduleID%>&equipmentCat=<%=request.getParameter("equipmentCat")%>&source=<%=source%><%=sType%>";
        } else {
            url="<%=context%>/ScheduleServlet?op=ViewSchedule&periodicID=<%=scheduleID%>&equipmentID=<%=request.getParameter("equipmentID")%>&source=<%=source%>";
        }
    ...
    <body>
        <DIV align="left" STYLE="color:blue;">
            ...
            if(source.equalsIgnoreCase("mainCat")){
                sBackURL = "&mainCatId=" + schWbo.getAttribute("mainCatId") + "&source="+source + sType;
            }else if(source.equalsIgnoreCase("Cat")){
                sBackURL = "&equipmentCat=" + request.getParameter("equipmentCat") + "&source="+source + sType;
            } else {
                sBackURL = "&equipmentID=" + request.getParameter("equipmentID") + "&source="+source;
            }
            %>
            ...
            <tbody id="planetData2">
                ...
                <TD nowrap CLASS="cell"  BGCOLOR="#D7FF82" STYLE="padding-left:10;<%=style%>">
                    <DIV ID="links">
                        ...
                        <A HREF="<%=context%>/ScheduleDocReaderServlet?op=ViewDocument&docType=<%=doc.getDocumentType()%>&docID=<%=(String) doc.getAttribute("docID")%>&metaType=<%=(String) doc.getAttribute("metaType")%>&scheduleID=<%=scheduleID%><%=sBackURL%>">
                            <%=docTitles[3]%>  
                        </A>
                ...
                <TD nowrap CLASS="cell"  BGCOLOR="#D7FF82" STYLE="padding-left:10;<%=style%>">
                    <DIV ID="links">
                        <A HREF="<%=context%>/ScheduleDocReaderServlet?op=DocDetails&docID=<%=doc.getAttribute("docID")%>&metaType=<%=doc.getAttribute("metaType")%>&scheduleID=<%=scheduleID%><%=sBackURL%>">
                            <%=docTitles[4]%>...
                        </A>
                    </DIV>
                </TD>
                ...
                <TD nowrap CLASS="cell"  BGCOLOR="#D7FF82" STYLE="padding-left:10;<%=style%>">
                    <DIV ID="links">
                        <A HREF="<%=context%>/ScheduleDocWriterServlet?op=GetEditForm&docID=<%=(String) doc.getAttribute("docID")%>&docTitle=<%=(String) doc.getAttribute("docTitle")%>&scheduleID=<%=scheduleID%><%=sBackURL%>">
                            <%=docTitles[5]%> 
                        </A>
                ...
                <TD nowrap CLASS="cell"  BGCOLOR="#D7FF82" STYLE="padding-left:10;<%=style%>">
                    <DIV ID="links">
                        <A HREF="<%=context%>/ScheduleDocReaderServlet?op=ConfirmDelete&docID=<%=(String) doc.getAttribute("docID")%>&scheduleID=<%=scheduleID%>&docTitle=<%=(String) doc.getAttribute("docTitle")%><%=sBackURL%>">
                        <%=docTitles[6]%>  
