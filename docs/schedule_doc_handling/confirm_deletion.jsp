...
<HTML>
    ...
    <SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
        ...
        %>
        document.ISSUE_FORM.action = "<%=context%>/ScheduleDocReaderServlet?op=Delete&docID=<%=docID%>&scheduleID=<%=scheduleID%>&equipmentCat=<%=request.getParameter("equipmentCat")%>&source=\'cat\'<%=sType%>";
        <%
        } else {
        %>
        document.ISSUE_FORM.action = "<%=context%>/ScheduleDocReaderServlet?op=Delete&docID=<%=docID%>&scheduleID=<%=scheduleID%>&equipmentID=<%=request.getParameter("equipmentID")%>&source=eqp";
        <%
        }
        %>
 ...
 function cancelForm()
        ...
        if(request.getParameter("equipmentCat") != null){
        %>
        document.ISSUE_FORM.action = "<%=context%>/ScheduleDocReaderServlet?op=ListDoc&scheduleID=<%=scheduleID%>&equipmentCat=<%=request.getParameter("equipmentCat")%>&source=\'cat\'<%=sType%>";
        <%
        } else {
        %>
        document.ISSUE_FORM.action = "<%=context%>/ScheduleDocReaderServlet?op=ListDoc&scheduleID=<%=scheduleID%>&equipmentID=<%=request.getParameter("equipmentID")%>&source=eqp";
        <%
        }
        %>
        document.ISSUE_FORM.submit();  
    ...
    <BODY>
        <DIV align="left" STYLE="color:blue;">
            ...
            <button    onclick="submitForm()" class="button"><%=save%>  <IMG SRC="images/del.gif">
            </button>
        </DIV> 
