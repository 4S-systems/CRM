...
<%
    ...
    String projectCordinate = (String) request.getAttribute("projectCordinate");
    Pattern pattern = Pattern.compile("\\(.*\\)$");
    String cord = "(21.422949491565994, 39.82664388997273)";
    if (!(projectCordinate == null || projectCordinate.equals(""))) {
        Matcher matcher = pattern.matcher(projectCordinate);

        if (matcher.find()) {
            cord = projectCordinate;
        }
    }
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function addMarker(location) {
                ...
                document.getElementById('coordinate').value=locati;
    ...
    <body onload="initialize()">
        ...
        <form method="POST" action="<%=context%>/ProjectServlet?op=insertProjectMap">
            <input type="submit" value="   حفظ المكان  "  class="button">
            <input name="coordinate" type="hidden" id="coordinate" value="<%=projectCordinate%>" size="92">
            <input name="projectId" type="hidden"  id="projectId" value="<%=projectId%>" size="92">
        </form>
