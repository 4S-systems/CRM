...
<c:set var="loc" value="en"/>
    ...
    <head>
        ...
        ArrayList<WebBusinessObject> RecipientList = request.getAttribute("RecipientList") != null ? (ArrayList<WebBusinessObject>) request.getAttribute("RecipientList") : null;
        String[] recepients = request.getAttribute("recepients") != null ? (String[]) request.getAttribute("recepients") : null;
       int size=0;
        if(recepients!=null)
            size=recepients.length;
        ...
        if (stat.equals("En")) {
            ...
            align = "center";
             ...
             title="Financial Transaction Search By Recipient" ;
             ByDocu="By Document Code";
             BytransCode="By Transaction Code";
             entrerNum="Choose The Recipient";
            ...
            title="بحث عن الحركات المحاسبيه بالمستفيد";
            ByDocu="بكود المستند";
            BytransCode="بالقيد المحاسبى";
            entrerNum="اختر المستفيد";
            DocNum="كود المستند";
            TransNum="كود الحركه";
            Date="تاريخ الحركه";
        ...
        <script  type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $("#contractorList").select2();
            <%if(size>0 ){
                    for (int i=0; i<recepients.length; i++) { %>
                    Values[<%= i %>] = "<%= recepients[i] %>"; 
                    <% } %>
                      $("#contractorList").val(Values).trigger('change');
              <% }%>
            ...
            function submitForm(){
                ...
                document.finanTransaction.action = "<%=context%>/FinancialServlet?op=financialTransactionSearchByRecipient&fromDate="+fromDate + "&toDate=" + toDate;
        ...
        <FORM NAME="finanTransaction" METHOD="POST">
            <FIELDSET class="set" style="width:98%;border-color: #006699">
                ...
                <table class="blueBorder" id="code2" align="center" dir="<%=dir%>" width="650" cellspacing="2" cellpadding="1"
                    ...
                    <tr>
                         <td class="blueHeaderTD"   style="text-align: center; font-size: 14px; border-width: 1px; border-color: white; width: 325px;"colspan="4">
                            <b><font size=3 color="white"><%=entrerNum%></font></b>
                        </td>
                         
                    </tr>
                    <tr>
                         <td     style="text-align: center; font-size: 14px; border-width: 1px; border-color: white; width: 325px;"colspan="4">
                            <select multiple id="contractorList" name="contractorList" class="set" align="center" STYLE="width: 50%; font-size: small; text-align: center;">
                                <sw:WBOOptionList wboList='<%=RecipientList%>' displayAttribute = "NAME" valueAttribute="ID" />
                            </select>
                         </td>
                    </tr>
