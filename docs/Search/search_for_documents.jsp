        ...
        <SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
            ...
            function getComplaints() {
                ...
                if ((beginDate === null || beginDate === "")) {
                    ...
                    document.COMP_FORM.action = "<%=context%>/SearchServlet?op=searchForDocuments&beginDate=" + beginDate + "&endDate=" + endDate + "&engineerID=" + engineerID + "&contractorID=" + contractorID;
            ...
            function showRequestedWorkItems(issueID,businessID,businessIDbyDate) {
                var url = "<%=context%>/SearchServlet?op=getExtractionWorkItems&issueID=" + issueID + "&businessID=" + businessID + "&businessIDbyDate=" + businessIDbyDate;
                openWindow(url);
            }
            function openWindow(url) {
                window.open(url, "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=1200, height=600");
            }
            
            function deleteRelation(issueID) {
                $.ajax({
                    type: 'POST',
                    url: "<%=context%>/ProjectServlet?op=deleteRequestDoc",
                        ...
                        <FIELDSET class="set" style="width:98%;border-color: #006699">
                            ...
                            <table align="center" style="margin-left: 20%; margin-right: 20%;" dir="rtl" width="60%" cellspacing="3" cellpadding="10">
                                ...
                                <tr>
                                    ...
                                    <td class="blueBorder blueHeaderTD" style="font-size:18px;" WIDTH="50%">
                                        <b> <font size=3 color="white"> المهندس المرسل </b>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="text-align:center" class="excelentCell"  valign="MIDDLE" >
                                        <select style="font-size: 14px;font-weight: bold; width: 80%" id="contractorID" >
                                            <option value="all">الكل</option>
                                            <sw:WBOOptionList wboList='<%=contractorsList%>' displayAttribute="name" valueAttribute="id" scrollToValue="<%=contractorID%>"/>
                                        </select>
                                    </td>
                                    <td  class="excelentCell"  style="text-align:center" valign="middle">
                                        <select style="font-size: 14px; font-weight: bold; width: 80%" id="engineerID" >
                                            <option value="all">الكل</option>
                                ...
                                <tr>
                                    <td style="text-align:center" class="excelentCell"  valign="MIDDLE">
                                        <select style="font-size: 12px; font-weight: bold; width: 80%" id="toFolder" name="toFolder">
                                            <sw:WBOOptionList wboList='<%=projects%>' displayAttribute="projectName" valueAttribute="projectID" scrollToValue="<%=engineerID%>"/>
                                        </select>
                                    </td>
                                ...
                                <table class="display"  id="indextable" align="center" DIR="<%=dir%>" WIDTH="100%" CELLPADDING="0" cellspacing="0">
                                    <thead>
                                        <tr>
                                            ...
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="13.5%"><b>رقم المستخلص</b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="13.5%"><b>المشروع</b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="10%"><b> <%=complaintCode%></b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="11.5%"><b>إضافة طلبات تسليم</b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="12%"><b>عرض طلبات تسليم</b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="10%"><b>عرض بنود الأعمال</b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="9%"><b>تاريخ التسجيل</b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="10%"><b>المسئول</b></TH>
                                            <TH STYLE="text-align:center; font-size: 16px; font-weight: bold; height: 25px" width="5%"><b><%=ageComp%></b></TH>
                                        </tr>
                                    </thead> 
                                    <tbody>
                                        <%
                                            ...
                                            for (WebBusinessObject wbo : data) 
                                            {
                                                int workItemsCount = 0;
                                                    ...
                                                    List<WebBusinessObject> requests = issueByComplaintMgr.getIssueComplaintsByComments(CRMConstants.CLIENT_COMPLAINT_TYPE_REQUEST_EXTRADITION, (String)wbo.getAttribute("issue_id"));
                                                    for (int i = 0; i < requests.size(); i++)
                                                    {
                                                        List<WebBusinessObject> requestedItems = RequestItemsDetailsMgr.getInstance().getByIssueId((String) requests.get(i).getAttribute("issue_id"));
                                                        workItemsCount += requestedItems.size();
                                                    }
                                        ...
                                        <tr id="row">
                                            ...
                                            <td><b><%=wbo.getAttribute("projectName")%></b></td>
                                            <td><b><%=wbo.getAttribute("nameOfProject") != null ? wbo.getAttribute("nameOfProject") : "---"%></b></td>
                                            ...
                                            <td><b><a style="color: grey;cursor: pointer" onclick="showRequestedWorkItems('<%=wbo.getAttribute("issue_id")%>','<%=wbo.getAttribute("projectName")%>','')">عرض بنود الأعمال</a>&nbsp;<font color="blue">(<%=workItemsCount%>)</font></b><img value="" onclick="" width="20" height="20" src="images/icons/view-request.png" style="float: left; margin-left: 4px; vertical-align: middle"/></td>
