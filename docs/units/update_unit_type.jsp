...
<html>
    <%
        ...
        String locationTypeID = unitTypeWbo != null && unitTypeWbo.getAttribute("locationTypeID") != null ? (String) unitTypeWbo.getAttribute("locationTypeID") : "";
        ...
        if (stat.equals("En")) {
            ...
            cancel = "Back To List";
            save = "Save";
            sStatus = "Updated Successfully";
            fStatus = "Fail To Update";
        } else {
            align = "right";
            dir = "RTL";
            ...
            save = "حفظ";
            fStatus = "لم يتم التحديث";
            sStatus = "تم التحديث بنجاح";
        }
    %>
    <head>
        <script language="JavaScript" type="text/javascript">
            function submitForm() {
                if (!validateData("req", this.UNIT_TYPE_FORM.type, "Please, select type.")) {
                    ...
                    document.UNIT_TYPE_FORM.action = "<%=context%>/ProjectServlet?op=getUpdateUnitType&update=true";
                    document.UNIT_TYPE_FORM.submit();
                }
            }
            function cancelForm() {
                document.UNIT_TYPE_FORM.action = "<%=context%>/ProjectServlet?op=listUnitTypes";
                document.UNIT_TYPE_FORM.submit();
            }
        </script>
    </head>
    <body>
        <form name="UNIT_TYPE_FORM" id="UNIT_TYPE_FORM" method="POST">
            <div align="left" STYLE="color:blue;">
                <button type="button" onclick="JavaScript: cancelForm();" class="button"><%=cancel%><img valign="bottom" src="images/leftarrow.gif"/> </button>
                    ...
                    <tr>
                        ...
                        <td colspan="3" class="blueBorder backgroundTable" >
                            <select name="type" ID="type" style="width: 270px; font-size: 16px;">
                                <sw:WBOOptionList wboList="<%=locationTypesList%>" displayAttribute="arDesc" valueAttribute="id" scrollToValue="<%=locationTypeID%>" />
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td colspan="3" class="blueBorder backgroundTable">
                            <input type="text" dir="<%=dir%>" name="name" ID="name" size="32" value="<%=unitTypeWbo.getAttribute("typeName")%>" maxlength="255" style="width: 270px;"/>
                        </td>
                    </tr>
                </table>
                <br /><br />
                <input type="hidden" name="id" ID="id" value="<%=unitTypeWbo.getAttribute("id")%>">
            </fieldset>
        </form>
    </body>
</html>     