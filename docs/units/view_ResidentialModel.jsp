...
<fmt:setBundle basename="Languages.Units.Units"  />
    ...
    <%
        ...
        // to get the projects under (Masharee3) Node 
        ...
        WebBusinessObject wbo = (WebBusinessObject) request.getAttribute("modelData");
        WebBusinessObject projectWbo12 = (WebBusinessObject) request.getAttribute("projectData");
    %> 
    <head>
        ...
        <script type="text/javascript" LANGUAGE="JavaScript">
            function cancelForm() {
                 document.PROJECT_FORM.action = "<%=context%>/UnitServlet?op=listResidentialModel";
            ...
            function updateForm(obj) {
               if (!validateData("req", this.PROJECT_FORM.project_name, "Please, enter Site Name.") || !validateData("minlength=3", this.PROJECT_FORM.project_name, "Please, enter a valid Site Name.")) {
                    ...
                    document.PROJECT_FORM.action = "<%=context%>/UnitServlet?op=updateResidentialModel&projectID=" + obj;
                    document.PROJECT_FORM.submit();
                }
            }
        ...
        <%if (request.getAttribute("modelData") != null) { %>
        <script type="text/javascript" LANGUAGE="JavaScript"> 
            $(function() {
                $("#min_price").val("<%=wbo.getAttribute("minPrice").equals("null") ? "" : (String)wbo.getAttribute("minPrice") %>");
                $("#max_price").val("<%=wbo.getAttribute("maxPrice") == null || wbo.getAttribute("maxPrice").equals("null") ? "" : (String)wbo.getAttribute("maxPrice")%>");
                $("#total_area").val("<%=wbo.getAttribute("totalArea") == null || wbo.getAttribute("totalArea").equals("null")   ? "" : (String)wbo.getAttribute("totalArea")%>");
                $("#net_area").val("<%=wbo.getAttribute("netArea") == null || wbo.getAttribute("netArea").equals("null") ? "" : (String)wbo.getAttribute("netArea")%>");
                $("#rooms_no").val("<%=wbo.getAttribute("roomsNo") == null ||wbo.getAttribute("roomsNo").equals("null") ? "" : (String)wbo.getAttribute("roomsNo")%>");
                $("#kitchens_no").val("<%=wbo.getAttribute("kitchensNo") == null ||wbo.getAttribute("kitchensNo").equals("null") ? "" : (String)wbo.getAttribute("kitchensNo")%>");
                $("#pathroom_no").val("<%=wbo.getAttribute("pathroomNo") == null ||wbo.getAttribute("pathroomNo").equals("null") ? "" : (String)wbo.getAttribute("pathroomNo")%>");
                $("#balcony_no").val("<%=wbo.getAttribute("balconyNo") == null ||wbo.getAttribute("balconyNo").equals("null") ? "" : (String)wbo.getAttribute("balconyNo")%>");
                if (<%=(String) wbo.getAttribute("garage")%> === 1) {
                    $("#garage").attr('checked', 'checked');
                }
                if (<%=(String) wbo.getAttribute("storage")%> === 1) {
                    $("#storage").attr('checked', 'checked');
                }
                if (<%=(String) wbo.getAttribute("elevator")%> === 1) {
                    $("#elevator").attr('checked', 'checked');
                }
                if (<%=(String) wbo.getAttribute("club")%> === 1) {
                    $("#club").attr('checked', 'checked');
                }

                $("#category").val("<%=(String) wbo.getAttribute("categoryName")%>");
               
            });

        </script>
        <%}%>
        <% if (request.getAttribute("projectData") != null) {%>
        <script type="text/javascript" LANGUAGE="JavaScript">
            $(function() {
                
                $("#eqNO").val("<%=(String) projectWbo12.getAttribute("eqNO")%>");
               
                $("#project_name").val("<%=(String) projectWbo12.getAttribute("projectName")%>");
                $("#project_desc").val("<%=(String) projectWbo12.getAttribute("projectDesc")%>");
                $("#mainProjectId").val("<%=(String) projectWbo12.getAttribute("mainProjId")%>");
                $("#submitBut").attr('onclick', 'JavaScript: updateForm("<%=(String) projectWbo12.getAttribute("projectID")%>");');
            });
        </script>
                ...
                <div class="row">
                    ...
                    <table width="50%" align="center">
                        <tr>
                            ...
                            %>
                            <td class="bar">
                                <b><font color="blue" size="3">تمت بنجاح</font></b>
                            </td>
                            <%
                            } else {
                            %>
                            <td class="td" class="bar">
                                <b><font color="red" size="3">لم تتم بنجاح</font></b>
                            </td>
                            <%
                                }
                            %>
                            ...
                            <form>
                                <fieldset class="set"  style="width:85% ; border-color: #006699;">
                                    <legend>
                                       ...
                                       <fmt:message key="vieweditresmodel" />
                                      </font>
                                      </legend>
                                    ...
                                    <table dir='<fmt:message key="direction" />' style="width: 100%">
                                        ...
                                        <option>----</option>
                                        <option value="أقتصادى">أقتصادى</option>    
                                        ...
                                        <option value="متوسط">متوسط</option>
                                            ...
                                            <tr>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td class="td">
                                                                        <TEXTAREA rows="5" name="project_desc" dir='<fmt:message key="direction" />' ID="project_desc" cols="26"></TEXTAREA>
                                                </td>
                                            </tr>
                    ...
                    <DIV align="center" STYLE="color:blue; width:50%">
                        ...
                        <button  id="submitBut" name="submitBut" onclick="JavaScript:  submitForm();" class="button">
                                ...
                                <input type="hidden"  size="12" value="UL" maxlength="255" name="max_price" id="max_price" onkeypress="JavaScript: return isNumber(event);"/> 
