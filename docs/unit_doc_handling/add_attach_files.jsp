...
<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
    ...
    function submitForm(){
        ...
        if(document.getElementById('docDate').value==""){
            ...
            document.EXTERNAL_ORDER_FORM.action = "<%=context%>/UnitDocWriterServlet?op=SaveDocByTree&projId=<%=projId%>&filterName=<%=filterName%>&filterValue=<%=filterValue%>&type=<%=type%>";
    ...
    <BODY>
        <FORM action=""  NAME="EXTERNAL_ORDER_FORM" METHOD="POST" ENCTYPE="multipart/form-data">
            ...
            <DIV align="left" STYLE="color:blue;">
                ...
                <button  onclick="JavaScript: cancelForm();" class="button" style="display: <%=type != null && type.equals("tree")?"":"none"%>"> <%=cancel%> <IMG alt="" SRC="images/cancel.gif"></button>
                <button  onclick="backForm('<%=context%>/ProjectServlet?op=ListProjects');" class="button" style="display: <%=type != null && type.equals("tree")?"none":""%>"><%=backToList%> <IMG VALIGN="BOTTOM"  HEIGHT="15" SRC="images/leftarrow.gif"></button>
