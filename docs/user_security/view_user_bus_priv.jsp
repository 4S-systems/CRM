...
<HTML>
    ...
    Vector<String> checkOps = (Vector<String>) request.getAttribute("checkOps");
     Vector checks =  (Vector) request.getAttribute("checks");
     Vector<WebBusinessObject> prevliges = (Vector<WebBusinessObject>) request.getAttribute("busPrevliges");
...
//            } else {
                document.USER_STORES_FORM.action = "<%=context%>/BussinessOpSecurityServlet?op=saveUserBussiness";
...
<!--                        <TR style="cursor: pointer" CLASS="act_sub_heading" onmousemove="this.className='selectedRow'" onmouseout="this.className='act_sub_heading'">
                            ...
                            for(WebBusinessObject prev : prevliges) {
                               ...
                               // String prevCode = (String) prev.getAttribute("bussId");
                                Boolean checkStore = Tools.isFound(prev.getAttribute("id").toString(), checkOps);
                            %>
                            <TR style="cursor: pointer" onmousemove="this.className='selectedRow'" onmouseout="this.className=''">
                                <TD STYLE="text-align:<%=align%>;padding-<%=align%>:45px;border-top-width: 0px">
                                    <%=prev.getAttribute("arDesc")%>
                                    <input type="hidden" id="arDesc" name="arDesc" value="<%=prev.getAttribute("arDesc")%>" />
                                    <input type="hidden" id="buss_id" name="buss_id" value="<%=prev.getAttribute("id")%>" />
                                </TD>
                                <TD style="text-align:center;border-top-width: 0px">
                                    <INPUT TYPE="CHECKBOX" ID="checkPrev" NAME="checkPrev" value ="<%=index%>" <%=(checkStore)? "checked" : ""%>>
                                </TD>
                            </TR>
                            <%index ++;
