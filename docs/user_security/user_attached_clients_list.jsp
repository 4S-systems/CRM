...
<%
    ...
    List<WebBusinessObject> listEmpAttachedEmployees = (List<WebBusinessObject>) request.getAttribute("clientsList");
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/UsersServlet?op=listUserAttachedClients&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                if(fieldName != null){
            ...
            function goToPage(pageIndex) {
                ...
                var url = "<%=context%>/UsersServlet?op=listAttachedClients&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                submitForm(url);
            }
            ...
            function send_Info(){
                ...
                var row;
                var insertedTable = window.opener.document.getElementById('userNamesTable');
                var check = document.getElementsByName('relatedEmpIds');
                var mgrId =document.getElementsByName('userId');
                //alert('mgrId '+mgrId);
...
//                var sodicId = document.getElementsByName('<%=fieldCode%>');
                ...
                var userId = document.getElementsByName('<%=fieldId%>');
...
//                var checkex = 0;   
                ...
                for(var i = 0; i < length; i++) {
                    ...
                    if(check[i].checked == true) {
                        ids[i]=check[i].value;
                        mgrs[i]=mgrId[i].value;
                       // removeEmp(check[i].value,mgrId[i].value);
//                        row = insertedTable.insertRow();   
//                        var classname="silver_odd_main";
//                        var cell0 = row.insertCell(0);
//                        cell0.className = classname;
//                        cell0.innerHTML = fullName[i].value +
//                            '<input type="hidden" name="userId" id="userId" value="' + userId[i].value + '" />';
            ...
            function removeEmp(ids,mgrs){
                //alert('length of emps='+ids.length);
                var url = "<%=context%>/UsersServlet?op=removeAttachedEmployees&empIds="+ids+"&mgrIds="+mgrs;
                submitForm(url)
            }
            
            function insert_Cells(row, order,index,fullName,userId, sodicId, comments){
            ...
            function defaultView(){
                ...
                var url = "<%=context%>/UsersServlet?op=listUserAttachedEmployees&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                
                submitForm(url);
            }
            
            function sendInfo(id, name,code) {
                //alert('entered    id='+id+'name = '+name);
                var insertedTable = window.opener.document.getElementById('tblData');
                // alert('insertedTable   == '+insertedTable.rows.length)
                var lastrow = insertedTable.rows.length;
                //alert('  opener.window.users[id]   ='+opener.window.clients[id])
                if(opener.window.clients[id]!=1) {
                    
                    var row;
                    
                    var count = 0;
                    if(!opener.window.clients[id]){
                        row = insertedTable.insertRow(1);
                        insert_Cells(row, lastrow, count + lastrow, name, code,id);
                        count++; 
                    } else { 
                        //alert('--- else test');
...
//                }
            ...
            function insert_Cells(row, order,index,name,client,id){
            //alert('insert_Cells')
                var classname="silver_odd_main";
                if(index%2==0)
                    classname="silver_even_main";
                ...
                cell7.innerHTML = '<span class="remove" onclick="remove(this, '+order+')" id="'+id+'" title="'+ id +'"  ></span>';
            ...
            <form method="post" name="LIST_USERS_FORM" action="">
                <DIV align="left" STYLE="color:blue;">
                    ...
                    <button  type=button STYLE=" " ONMOUSEDOWN="send_Info();"  style="background: url(images/buttonBlue.jpg) repeat-x top center; border: 1px solid #999;
                             ...
                             -webkit-border-radius: 5px; font-size: 13px;  width: 90px; hieght:60px; " > <font color="black" ></font>Remove</button>
                        ...
                        <table width="500">
                            ...
                            <tr>
                                ...
                                <th class="thTitle" style="width: 20px;">
                                    <input type="checkbox" id="checkAll" onclick="CheckAll(this.id, 'relatedEmpIds');" value=""/>
                                </th>
                                <%}%>
                                <th class="thTitle">
                                    <%=code%>
                            ...
                            <%
                                ...
                                for (int i = 0; i < listEmpAttachedEmployees.size(); i++) {

                                    wbo = (WebBusinessObject) listEmpAttachedEmployees.get(i);
                                    ...
                                    //mgrId= (String) wbo.getAttribute("mgrId");
                                    iTotal++;


                            %>
                            <%if (selectionType.equalsIgnoreCase("single")) {
                                    event = "onclick=\"return sendInfo('" + idValue + "', '" + nameValue + "','"+codeValue+"')\"";
                                }%>
                            <tr>
                                ...
                                <%if (selectionType.equalsIgnoreCase("multi")) {%>
                                <td class="thTitle">
                                    <input type="checkbox" name="relatedEmpIds" value="<%=idValue%>"/>
                                    <input type="hidden" name="userId" value="<%=mgrId%>"/>
                                </td>
                                <%}%>
                                <td class="td2" style="width: 20px;">
                                    <a  href="#" <%=event%>>
