...
<%
    ...
    List<WebBusinessObject> listUnattachedEmployees = (List<WebBusinessObject>) request.getAttribute("clientsList");
    WebBusinessObject wbo = null;
    String fieldId = "id";
    String fieldName = "name";
    String fieldCode = "clientNO";

    String tableNameField = "NAME", tableIdField = "ID", tableCodeField = "CLIENT_NO";
%>
<html>
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var dates = $( "#fromDate" ).datepicker({
                    ...
                    onSelect: function( selectedDate ) {
                        ...
                        dates.not( "#fromDate" ).datepicker( "option", "minDate", date );
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/UsersServlet?op=listUnattachedClients&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                if(fieldName != null){
            ...
            function goToPage(pageIndex) {
                ...
                //alert('after catch');
                var url = "<%=context%>/UsersServlet?op=listUnattachedClients&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType+ "&userId=" + user;
                submitForm(url);
            }
...
//                //alert('entered to send');
             ...
             // alert('length='+length)
                var client = document.getElementsByName('<%=fieldId%>');
                var clientNo = document.getElementsByName('<%=fieldCode%>');
                var name = document.getElementsByName('<%=fieldName%>');
                var commentsArr = document.getElementsByName('comments');
                var fromDateArr = document.getElementsByName('fromDate');
                var relationTypeArr = document.getElementsByName('relationType');
...
//                        alert('test');
                    //alert('values: client= '+client[i].value);
                        if(!opener.window.clients[client[i].value]){
//                            alert('test');
                           ++insertCount;
                            row = insertedTable.insertRow(insertCount);   
//                     alert('values: row= '+row+' lastrow= '+lastrow+' count= '+count);
                            insert_Cells(row, lastrow, count + lastrow, name[i].value, client[i].value, clientNo[i].value, commentsArr[i].value, fromDateArr[i].value,relationTypeArr[i].value);
            ...
            function insert_Cells(row, order,index,name,client, clientNo, comments,fromDate,relationType){
                ...
                cell0.innerHTML = '<input type="hidden" name="index" id="index" size="10" value="'+index+'" /><span id="order" name="order" value="'+index+'">' +index+ "</span><input type=\"hidden\" name=\"client\" value="+ client +" />";
               
                
                cell1.className =classname;
                cell1.innerHTML = clientNo;
                
                cell2.className = classname;
                cell2.innerHTML = name+
                    '<input type="hidden" name="name" id="name'+order+'" value="'+name+'" />';
                ...
                cell4.innerHTML = '<input class="datePicker"  type="text" style="width: 100px;" name="fromDate" id="fromDate" size="20" value="'+fromDate+'" /><img alt=""  src="images/showcalendar.gif" onmouseout="UnTip()"  onmouseover="Tip(calenderTip,CENTERMOUSE, true, OFFSETX, 0, BGCOLOR,"'+ '#FFFF99'+'"  , FONTCOLOR, "'+ 'BLACK'+'", FONTSIZE, "'+ '10pt'+'", FONTWEIGHT ,"'+ 'BOLD'+'", SHADOW, true, SHADOWWIDTH, 7, TITLE ,"'+'Display Calender Help'+'",TITLEALIGN ,"'+'center'+'" ,TITLEFONTCOLOR ,"'+'black'+'", TITLEBGCOLOR ,"'+'#6699FF'+'")" />';
                
                cell5.className = classname;
                cell5.innerHTML = '<select name="relationType" id="relationType" value="'+relationType+'" ><option value="Support">خدمة عملاء</option><option value="Deliver">مسئول تسليمات</option><option value="Accounting">مسئول حسابات</option></select>';
                
                cell6.className = classname;
                cell6.innerHTML = '<div id="save'+index+'" class="save" style="background-position: bottom;" onclick="upsertUserClientRelation(this, '+index+')"></div>';
                  
                cell7.className = classname;
                cell7.innerHTML = '---';
                              
                opener.window.clients[client]=1;
                    ...
                    <div>
                        <table width="500">
                            ...
                            <%
                                ...
                                String event = "single";
                                String idValue, nameValue, codeValue,relationType;
                                        String comments="";
                                        String fromDate=nowTime;
                                for (int i = 0; i < listUnattachedEmployees.size(); i++) {
                                    ...
                                    fromDate= (String) wbo.getAttribute("fromDate");
                                    relationType= (String) wbo.getAttribute("relationType");
                                    iTotal++;
