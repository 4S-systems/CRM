...
<%
    ...
    String pgTyp = request.getParameter("pgTyp") != null ? request.getParameter("pgTyp") : "";
    ...
    String fieldCode = "EMP_ID";
    String fieldName = "EMP_NAME";
    if (stat.equals("Ar")) {
        code = "&#1575;&#1604;&#1603;&#1608;&#1583;";  //
        name = "&#1575;&#1604;&#1573;&#1587;&#1605;";
        listLabel = "قائمة المديرين";
        list_title_page = "قائمة المديرين";
...
<html>
    <head>
		...
		var pgTyp = '<%=pgTyp%>';
		var empID = '<%=empID%>';
                var url = "<%=context%>/UsersServlet?op=getManagerForDepartment&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType + "&itemNo=<%=itemNo%>&pgTyp=" + pgTyp + "&empID=" + empID;
                if(fieldName != null){
            ...
            function goToPage(pageIndex) {
               // alert('page index== '+pageIndex);
                ...
                var url = "<%=context%>/UsersServlet?op=getManagerForDepartment&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType + "&itemNo=<%=itemNo%>";
            ...
            function defaultView(){
                ...
                var url = "<%=context%>/UsersServlet?op=getManagerForDepartment&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType ;
                if(fieldName != null){
                    url += "&fieldName=" + fieldName;
                    if(fieldValue != null || fieldValue != ""){
		...
		if(pgTyp != null && pgTyp == 'cngMgr'){
		    window.opener.open("<%=context%>/UsersServlet?op=chngEmpMgr&empID=" + empID + "&nwMgrID=" + id);
		     window.close();
                } else if(opener.window.users[id]!=1) {
                    ...
                    window.opener.document.getElementById('secondEmpSearchBtn').disabled = false;
                     window.opener.document.getElementById('listEmps').disabled = false;
                        ...
                        <table width="500">
                            ...
                            <%
                                ...
                                for (int i = 0; i < data.size(); i++) {
                                    ...
                                    String departmentName=wbo.getAttribute("departmentName").toString();
//                                    if (codeValue.equalsIgnoreCase((String) request.getAttribute("itemId"))) {
//                                        continue;
//                                    }
                                    String nameValue = "";
                                    try {
                                        nameValue = wbo.getAttribute("userName").toString();
                                    } catch (Exception ex) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"> <%=departmentName%> </font> </b>
                                    </a>
                                </td>
                                <td>
                                    <a  href="#" <%=event%>>
