<%-- 
    Document   : addBusinessItems
    Created on : Aug 29, 2017, 4:08:26 PM
    Author     : fatma
--%>
...
<html>
    ...
    <head>
        ...
        <title> Add Business Items </title>
        ...
        <script>
                                    ...
                                    Done: function() {
                                        ...
                                        $(this).find(':checkbox:checked').each(function() {
                                            ...
                                            $('#row' + counter).append("<TD STYLE=\"text-align: center\" BGCOLOR=\"#DDDD00\" nowrap  CLASS=" + className + " ><b><font size=\"2\" color=\"#000080\" style=\"text-align: center;\">" + itemCode + "</font></b></TD>")
                                                                ...
                                                                .append("<td style=\"text-align: center; border-left-width: 0px\" bgcolor=\"#DDDD00\" nowrap=\"\" class=\"silver_even_main\"><input type=\"text\" id=\"total" + counter + "\" name=\"total\" value=\"" + rateHour + "\" style=\"width: 100%\" readonly></td>")
            ...
            var rhh=Number($("#rateHour" + rowNum).val()) ;
            if(rhh == null)
            {
                rhh=0;
                var total = rhh* Number($("#hours" + rowNum).val());
            ...
            function saveBItems(){
                if (document.getElementById('row0') === null) {
                    alert(" Choose One Business Item At Least ");
                ...
                $.ajax({
                    url: "<%=context%>/IssueServlet?op=addBusinessItems",
                    data: {
                        issueId: <%=issueId%>,
                        reqType: "jobOrder",
                    ...
                    success: function(data, textStatus, jqXHR) {
                        location.reload();
                        alert(" Busniss Item Has Been Added ");
                    }
                });
                //document.BUSNISS_ITEMS_FORM.action = "<%=context%>/IssueServlet?op=addBusinessItems&issueId=<%=issueId%>&clientComplaintType=<%=CRMConstants.CLIENT_COMPLAINT_TYPE_REQUEST_EXTRADITION%>&reqType=jobOrder&rowsNum=" + counter;
                //document.BUSNISS_ITEMS_FORM.submit();
            }
            ...
            function deleteItm(itmID){
                if(comStatus == "7"){
                    ...
                    $.ajax({
                        ...
                        success: function() {
                            location.reload();
                            alert(" Business Item Has Been Deleted ");
                        }
                    });
                }
            }
        ...
        <FIELDSET class="set" style="width:80%;border-color: #006699">
            <legend align="center">
                <font color="blue" size="6">
                     <fmt:message key="addBWork" /> 
                </font>
            </legend>
            
            <form name="BUSNISS_ITEMS_FORM" method="post" enctype="multipart/form-data">
                ...
                <table id="requestedItems" class="excelentCell" dir="ltr" width="80%" CELLPADDING="0" CELLSPACING="0" style="margin-left: 5px; margin-bottom: 5px">
                    <thead>
                        <tr>
                            <td class="blueBorder blueHeaderTD" width="8%"> Code </td>
                            <td class="blueBorder blueHeaderTD" width="31%"> Name </td>
                            <td class="blueBorder blueHeaderTD" width="10%"> R/H </td>
                            <td class="blueBorder blueHeaderTD" width="10%"> Hours </td>
                            <td class="blueBorder blueHeaderTD" width="10%"> Total </td>
                            <td class="blueBorder blueHeaderTD" width="31%"> Comment </td>
                            <td class="blueBorder blueHeaderTD" width="31%"> Delete </td>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        <%
                            if(busItmLst != null){
                                for (WebBusinessObject busItmWbo : busItmLst) {%>
                                    <tr id="row">
                                        ...
                                        <td style="text-align: center" bgcolor="#DDDD00" nowrap="" class="silver_even_main">
                                             <%String amnt = ((String)busItmWbo.getAttribute("option3")).contains("-") ? "0" : (String)busItmWbo.getAttribute("option3");%> 
                                            <%=amnt%> 
                                        </td>

                                        <td style="text-align: center" bgcolor="#DDDD00" nowrap="" class="silver_even_main">
                                             <%=busItmWbo.getAttribute("hourNum")%> 
                                        </td>
                                        ...
                                        <td style="text-align: center; border-left-width: 0px" bgcolor="#DDDD00" nowrap="" class="silver_even_main">
                                            <textarea type="text" style="width: 100%; border: none; cursor: not-allowed;" cols="10" readonly> <%=busItmWbo.getAttribute("note")%> </textarea>
                                        </td>
                                        
                                        <td style="text-align: center; border-left-width: 0px" bgcolor="#DDDD00" nowrap="" class="silver_even_main">
                                            <img src="images/icons/delete_ready.png" alt=" Delete Item " style="cursor: pointer; width: 20px; height: 20px;" onclick="deleteItm(<%=busItmWbo.getAttribute("itmID")%>);"/>
                                        </td>
                                    </tr>
                        <%      }
                            }
                        ...
                        %>
                                        <tr id="row<%=counter%>">
                                            ...
                                            <td style="text-align: center; border-left-width: 0px" bgcolor="#DDDD00" nowrap="" class="silver_even_main">
                                                <input type="text" id="total<%=counter%>" name="total" value="" style="width: 100%" readonly>
                                            </td>
