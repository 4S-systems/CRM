...
<%
            ...
            if (stat.equals("Ar")) {
                code = "&#1575;&#1604;&#1603;&#1608;&#1583;";  //
                name = "&#1575;&#1604;&#1573;&#1587;&#1605;";
                listLabel = " &#1602;&#1575;&#1574;&#1605;&#1577; &#1605;&#1585;&#1575;&#1603;&#1586; &#1575;&#1604;&#1578;&#1603;&#1604;&#1601;&#1577;";
                list_title_page = " &#1602;&#1575;&#1574;&#1605;&#1577; &#1605;&#1585;&#1575;&#1603;&#1586; &#1575;&#1604;&#1578;&#1603;&#1604;&#1601;&#1577;";
                ...
                code = "Code";  //
                name = "Name";
                listLabel = "Cost Center List";
                list_title_page = "Cost Center List";
            ...
            List<WebBusinessObject> costCenters = (List<WebBusinessObject>) request.getAttribute("costCenters");
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function refreshForm() {
                ...
                var url = "<%=context%>/AssignedIssueServlet?op=listCostCenters&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
                if(fieldName != null){
                    url += "&fieldName=" + fieldName;
                    if(fieldValue != null || fieldValue != ""){
                        url += "&fieldValue=" + fieldValue + "&operation=" + operation + "&rowIndex=<%=rowIndex%>";// + "&newRelation=<%--=newRelation--%>";
                    }
                }
                submitForm(url);
            }
            ...
            function checkDirction(){
                var dir = 'rtl';//defualt direction for arabic
                return dir;
            }
            function goToPage(pageIndex) {
                ...
                var url = "<%=context%>/AssignedIssueServlet?op=listCostCenters&Index=" + pageIndex + "&rowsPerPage=" + rowsPerPage + "&selectionType=" + selectionType;
            ...
            function defaultView(){
                ...
                if(fieldName != null){
                    url += "&fieldName=" + fieldName;
                    if(fieldValue != null || fieldValue != ""){
                        url += "&fieldValue=" + fieldValue + "&rowIndex=<%=rowIndex%>";
                    }
                }
                submitForm(url);
            }

            function sendCostCenterInfo(id, name, rowIndex){
                var id=id;
                var name = name;
                if(rowIndex != ''){
                    window.opener.document.getElementById('costCode_' + rowIndex).value = id;
                    window.opener.document.getElementById('costName_' + rowIndex).value = name;
                } else {
                    window.opener.document.getElementById('costCode').value = id;
                    window.opener.document.getElementById('costName').value = name;
                }
                window.close();
            }
            /*fcName = 'costName_' + rowIndex;
                fcCode = 'costCode_' + rowIndex;

                window.opener.document.<%=formName%>.fname.value = name;
                window.opener.document.<%=formName%>.fcode.value= id;*/
        </script>
    </head>
    <body>
                        ...
                        <TABLE ID="tableSearch" class="blueBorder" ALIGN="center" DIR="<%=dir%>" CELLPADDING="0" CELLSPACING="0" width="90%">
                            <%
                                        ...
                                        if (conditions.size() > 0) {
                                            for (int j = 0; j < conditions.size(); j++) {
                                                ...
                                                if (field_name.equalsIgnoreCase("COSTCODE")) {
                                                    selectedCode = "selected";
                                                } else if (field_name.equalsIgnoreCase(costName)) {
                            ...
                            <TR>
                                <TD CLASS="" STYLE="text-align: <%=textAlign%>;font-size:15px;">
                                    <select id="fieldName">
                                        <option value ="COSTCODE" <%=selectedCode%>><%=code%></option>
                                        <option value ="<%=costName%>" <%=selectedName%>><%=name%></option>
                                    </select>
                                </TD>
                        ...
                        <table width="500">
                            ...
                            <%
                                ...
                                for (int i = 0; i < costCenters.size(); i++) {
                                    ...
                                    wbo = (WebBusinessObject) costCenters.get(i);
                                    String costCode = wbo.getAttribute("COSTCODE").toString();
                                    String cost_name = null;
                                    try{
                                       cost_name = wbo.getAttribute(costName).toString();
                                    } catch(Exception ex){
                                        if(costName.equalsIgnoreCase("COSTNAME")){
                                            cost_name = "لا يوجد إسم عربي";
                                        } else {
                                            cost_name = "No English Name Provided";
                                        }
                                    }
                                    iTotal++;

                                    event = "onclick=\"return sendCostCenterInfo('" + costCode + "', '" + cost_name + "' , '" + rowIndex + "')\"";
                            %>
                            <tr>
                                ...
                                <td>
                                    <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"> <%=costCode%> </font> </b>
                                    </a>
                                </td>
                                <td>
                                    <a  href="#" <%=event%>>
                                        <b style="text-decoration: none"><font color="black" size="3"> <%=cost_name%> </font> </b>
                                        <input type="hidden" name="costName" value="<%=cost_name%>"/>
