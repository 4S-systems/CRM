...
<HTML>
    ...
    String search,AddCode,add,AddName,addNew, isMainEq,tCost,code,name,price,count,cost,Mynote,del,scr,sBackToList,attachTask;
    String updateParts;
    if(stat.equals("En")){
        ...
        attachTask="Attach Direct Purchase spare parts to task";
        ...
        attachTask="&#1585;&#1576;&#1591; &#1575;&#1605;&#1585;&#1575;&#1604;&#1588;&#1594;&#1604; &#1576;&#1602;&#1591;&#1593; &#1594;&#1610;&#1575;&#1585; <br> &#1605;&#1606; &#1575;&#1604;&#1588;&#1585;&#1575;&#1569; &#1575;&#1604;&#1605;&#1576;&#1575;&#1588;&#1585;"; 
   ...
   var count=0;
    ...
    return;
     ...
     var q=document.getElementsByName('qun');
      ...
      if(check == "0" || check=="" ){
       ...
       document.WORKSHOP_FORM.action = "<%=context%>/AssignedIssueServlet?op=saveconfigParts&projectID=<%=wbo.getAttribute("projectName").toString()%>&assignNote=<%=failureCode%>&uID=<%=uID%>&filterName=StatusProjectListAll&filteValue=<%=filterValue%>&page=localspares&isDirectPrch=1&attachedOn="+txt;
       document.WORKSHOP_FORM.submit();  
       }else
       {
         document.WORKSHOP_FORM.action = "<%=context%>/AssignedIssueServlet?op=saveconfigParts&projectID=<%=wbo.getAttribute("projectName").toString()%>&assignNote=<%=failureCode%>&uID=<%=uID%>&filterName=StatusProjectListAll&filteValue=<%=filterValue%>&page=localspares&isDirectPrch=1";
         document.WORKSHOP_FORM.submit(); 
       }
    }
    ...
    return false;
     ...
     return;
        ...
        C6.innerHTML = "<input type='text' name='note' ID='note' value='Add your Note'>";
         if(test.match("attached")){
            C7.innerHTML = "<input type='checkbox' name='checkattachEq' ID='checkattachEq'>";
    ...
    var url2 = "<%=context%>/TaskServlet?op=getLocalSpareData&key=" + code[x].value;
...
function noCodeFound( ) {
 ...
 window.onload = function () {
        ...
        { 
             var url = "<%=context%>/TaskServlet?op=listLocalSpares";
           //var url = "<%=context%>/ajaxGetItrmName?page=localparts&key=aa";
            if (window.XMLHttpRequest)
            { 
                req = new XMLHttpRequest(); 
            } 
 ...
 function f()
    ...
    <BODY>
        ...
        itemData=localStoresItemsMgr.getOnArbitraryKey(attValue,"key1");
                        WebBusinessObject webData=(WebBusinessObject)itemData.get(0);
                        %>
                        
                        <TD><input type="text" readonly name="code" id="code" value="<%=webData.getAttribute("itemCode").toString()%>">
                            
                        </TD>
                        
                        <TD><input type="text" readonly name="name1" id="name1" value="<%=webData.getAttribute("itemName").toString()%>">
