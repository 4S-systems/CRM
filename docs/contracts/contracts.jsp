...
<html>
    <%
        ...
        ArrayList<WebBusinessObject> unitsList = reservationMgr.getReservedUnits(fromDateD, toDateD, null, null);
        ...
        String cancelButtonLabel,ClientASS;
        if (stat.equals("En")) {
            dir = "LTR";
    ...
    <head>
        ...
        <SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
            ...
            function checkBeforConfirm(ReservationID,clientID,unitStatus)
            {
                var text="";
                $.ajax({
                        type: "post",
                        url: "<%=context%>/UnitServlet?op=checksBeforConfirmation",
                        data: {
                        ...
                        success: function(jsonString) {
                            ...
                            if (eqpEmpInfo.PaymentExist == 'no') {
                               text=text+"لايوجد خطة دفع "+"\n";
                            } 
                            if (eqpEmpInfo.elapsedM == 'no') {
                               text=text+"لم يتم تسديد القيمه الكامله لمقدم الحجز ومقدم التعاقد "+"\n";
                            } 
                           text=text+"هل ترغب فى التأكيد؟ " ;
                           if (confirm(text))
                           {
                               changeStatus(ReservationID, '<%=CRMConstants.RESERVATION_STATUS_CONFIRM%>', 'confirm', unitStatus);
                           }else{
                           }
                        }
                    });
            ...
            function navigateToClientReport(clientId) {
                var url = "<%=context%>/FinancialServlet?op=clientStatOfAcc&contractor=" + clientId;
                openInNewWindow(url);
            }
                    ...
                    <TABLE ALIGN="center" dir="<%=dir%>" id="unitsList" style="width:100%;">
                        ...
                        <tbody>
                            ...
                            <TR>
                                ...
                                <td id="3<%=wbo.getAttribute("id")%>" class="<%=className%>" style="cursor: pointer" nowrap>
                                    <table width="100%" cellpadding="0" cellspacing="0">
                                        <tr>
                                            <td class="td" style="width:30%"  onclick="JavaScript : navigateToClient('<%=wbo.getAttribute("clientId")%>')" nowrap>
                                                <B><%=wbo.getAttribute("clientName")%></B>
                                            </td>
                                            <td class="td" style="width:30%"  onclick="JavaScript : navigateToClient('<%=wbo.getAttribute("clientId")%>')">
                                                <img src="images/client_details.jpg" width="30" style="float: left;" title="تفاصيل" />
                                            </td>
                                            <td class="td" style="width:30%"  onclick="JavaScript : navigateToClientReport('<%=wbo.getAttribute("clientId")%>')">
                                               <img src="images/finical-rebort.png" width="30" style="float: left;" title="كشف حساب العميل" />  
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                ...
                                <td id="8<%=wbo.getAttribute("id")%>" class="<%=className%>">
                                    ...
                                    %>
                                    <B>
                                        <a href="JavaScript: planPDF('<%=wbo.getAttribute("clientId")%>', '<%=wbo.getAttribute("projectId")%>');">
                                ...
                                <td id="action<%=wbo.getAttribute("id")%>" class="<%=className%>" nowrap>
                                    ...
                                    <input type="button" value="Confirm" onclick="JavaScript: checkBeforConfirm('<%=wbo.getAttribute("id")%>','<%=wbo.getAttribute("clientId")%>', '<%=wbo.getAttribute("unitCurrentStatus")%>')"/>
