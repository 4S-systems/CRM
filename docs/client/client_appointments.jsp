    ...
    <HEAD>
        <%
            ...
            String style = null, lang, langCode, title, fromD, toD, dep, emp, classification,
                    typ, all, code, name, calls, meeting, srch, tel, mail, internet,
                    notes, regDate, secDate, appDate, app, status, call, met,
                    reslt, sysDur, actDur, noCom, dirApp, metDate, answered, notAnswered,
                    attended, notAttended, appointDate, registrationDate, campaign, sDur,aDur, mobile, type;

            if (stat.equals("En")) {
                align = "left";
                dir = "LTR";
                style = "text-align:right";
                title = "Clients Calls/Meetings";
                fromD = "From Date";
                toD = "To Date";
                dep = "Department";
                emp = "Employee";
                ...
                registrationDate = "Call/Meeting Date";
                campaign = "Campaign";
                ...
                registrationDate = "تاريخ المكالمة/المقابلة";
                campaign = "الحملة";
                sDur = "مدة النظام";
                aDur = "المدة الفعلية";
                mobile = "رقم الموبايل";
...
//            $("#slectEmp").select2();
    ...
    <body>
        <fieldset class="set" style="border-color: #006699; width: 90%;margin-top: 10px;border-radius: 5px;">
            <form name="SEARCH_CLIENT_FORM" action="<%=context%>/AppointmentServlet?op=getAllClientAppointments" method="POST">
                        ...
                        <td bgcolor="#dedede"  style="text-align:center" valign="middle" colspan="2">
                            <div id="mydiv">
                                <select style="font-size: 14px;font-weight: bold; width: 170px;" id="appointmentType" name="appointmentType"
                                    ...
                                    <option value="call" <%=appointmentType.equals("call") ? "selected" : ""%>><%=calls%> / <%=internet%></option>
                                    ...
                                    <td>
                                        <a id="pdf" href="<%=context%>/AppointmentServlet?op=clientAppointmentPDF&userID=<%=createdBy%>&departmentID=<%=departmentID%>&appointmentType=<%=request.getAttribute("appointmentType")%>&fromDate=<%=fromDate%>&toDate=<%=toDate%>&rateID=<%=rateID%>&result=<%=result%>&campaignID=<%=campaignID%>&dateType=<%=dateType%>">
                                            <img style="margin: 3px" src="images/icons/pdf.png" width="24" height="24"/>
                                             ...
                                             <%
                                        if (privilegesList.contains("EXCEL")) {
                                    %>
                                        <a id="pdf" href="<%=context%>/AppointmentServlet?op=clientAppointmentExcel&userID=<%=createdBy%>&departmentID=<%=departmentID%>&appointmentType=<%=request.getAttribute("appointmentType")%>&fromDate=<%=fromDate%>&toDate=<%=toDate%>&rateID=<%=rateID%>&result=<%=result%>&campaignID=<%=campaignID%>&dateType=<%=dateType%>">
                                            <img style="margin: 3px" src="images/excelIcon.png" width="24" height="24"/>
                                        </a>
                                            <%}%>
                                    </td>
                                </tr>
                                ...
                                <TD>
                                    ...
                                    <a target="_SELF" href="<%=context%>/ClientServlet?op=clientDetails&amp;clientId=<%=wbo.getAttribute("clientID")%>">
                                                    <img class="icon" src="images/client_details.jpg" width="23" height="23" style="float: left;" />
                                                         <!--onmouseover="JavaScript: getFollowupCounts('<%=wbo.getAttribute("clientID")%>', this);"--> 
                                                </a>
                            ...
                            <TR style="padding: 1px;">
                                ...
                                <TD colspan="7">
                                    <table style="width: 98%; margin: auto;" cellpadding="0" cellspacing="0">
                                        ...
                                        <%
                                            int counter = 0;
                                            String clazz, note, appointmentDate, currentStatusDate, tempType;
                                            for (WebBusinessObject appointmentWbo : appointmentsList) {
                                                ...
                                                tempType = appointmentWbo.getAttribute("option2") != null ? (String) appointmentWbo.getAttribute("option2") : "";
                                                switch (tempType) {
                                                    case "call":
                                                        tempType = "Internet";
                                                        break;
