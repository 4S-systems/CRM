...
<HTML>
    <%
        ...
        String dir = null, from, to, clntNm, callDt, callYear, callMonth, branch, calStat, calRed, tknTim, contactAft,
                note, mob, phone, tel, empNm, inDate;
        if (stat.equals("En")) {
            align = "center";
            xAlign = "right";
            dir = "LTR";
...
//                    },
                    ...
                    "columnDefs": [
                        {
                            ...
                            "data": null,
                            "defaultContent": "<img id='star' style='display:none;width:30px;height:30px;vertical-align:middle;'/> <a target='_blanck' id='goTo'><img src='images/client_details.jpg' style='vertical-align:middle;'/></a>"
                    ...
                    drawCallback: function () {
                        ...
                        $tbody.find('tr').each(function () {
                            ...
                            var isEmpty = true;
                            $tds.each(function () {
                                if ($.trim($(this).text()) !== '' || $(this).find('img,a').length > 0) {
                                    isEmpty = false;
                                }
                            });
                            if (isEmpty) {
                    ...
                    "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                        ...
                        if (aData[9] == "اهملت" || (aData[9] == "مفتوحة" && diffDays > 1)) {
                            ...
                            $('#star', nRow).attr('src', 'images/icons/bookmarks.png').show();
                        }
                        $('.sorting_1', nRow).css('background-color', 'white');
                        $('#goTo', nRow).attr("href", "<%=context%>/ClientServlet?op=clientDetails&clientId=" + aData[4]);
                        $('#pdf', nRow).attr("href", "JavaScript: printClientInformation('" + aData[3] + "')");
                    }
                });
            <%} else {%>
                var table = $('#clients').DataTable({
                    ...
                    "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                        ...
                        $('.sorting_1', nRow).css('background-color', 'white');
                        $('#goTo', nRow).attr("href", "<%=context%>/ClientServlet?op=clientDetails&clientId=" + aData[12]);
                        $('#goToWith', nRow).attr("href", "<%=context%>/DatabaseControllerServlet?op=dragClientForm&searchBy=unitNo&searchByValue=" + aData[13]);
                        $('#pdf', nRow).attr("href", "JavaScript: printClientInformation('" + aData[12] + "')");
                    }
                });
            <%}%>
            });
        ...
        <style type="text/css">
            ...
            #clients td.col-actions img#star:not([src]),
            #clients td.col-actions img#star[src=""] {
