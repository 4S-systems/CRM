...
<%
    ...
    String sCallTyp = (String) request.getAttribute("sCallTyp");
    ...
    if (stat.equals("En")) {
        ...
        lang = "&#1593;&#1585;&#1576;&#1610;";
        langCode = "Ar";
        title = " Appointments of Classified Clients for Communication Channels(Answered Calls)";
        ...
        noGroup = "No Groups";
        grop = "Groups";
        appDate = "Client Appointment Date";
    } else {
        dir = "RTL";
        align = "right";
        style = "text-align:Right";
        lang = "English";
        langCode = "En";
        title = "متابعات العملاء المصنفة بالنسبة لقنوات الإتصال- المكالمات المجابة";
        fromDateStr = " من تاريخ ";
        toDateStr = " إلى تاريخ ";
        callTyp = "نوع المكالمة";
        rate = "التصنيف";
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function exportToExcel() {
                document.SEARCH_APP_FORM.action = "<%=context%>/ProjectServlet?op=appOfClientClassOfComChanExcel";
    ...
    <body>
        <fieldset class="set" style="border-color: #006699; width: 90%;margin-top: 10px;border-radius: 5px;">
            <form name="SEARCH_APP_FORM" action="<%=context%>/ProjectServlet?op=appOfClientClassOfComChan" method="POST">
                ...
                <table ALIGN="center" DIR="<%=dir%>" WIDTH="50%" CELLSPACING=2 CELLPADDING=1>
                    ...
                    <tr>
                        <td style="text-align:center" bgcolor="#dedede"  valign="MIDDLE">
                            <input id="fromDate" name="fromDate" type="text" value="<%=fromDate%>" title="<%=appDate%>" readonly />
                        </td>
                        <td  bgcolor="#dedede"  style="text-align:center" valign="middle">
                            <input id="toDate" name="toDate" type="text" value="<%=toDate%>" title="<%=appDate%>" readonly />
                        </td>
                        <td  bgcolor="#dedede"  style="text-align:center" valign="middle">
                    ...
                    <tr>
                        <td  bgcolor="#dedede"  style="text-align:center" valign="middle">
                            <select id="sCallTyp" name="sCallTyp" STYLE="width: 225px;font-size: medium; font-weight: bold;" onchange="JavaScript: callResultsChng()">
                                <option value="<%=CRMConstants.CALL_RESULT_INBOUNDCALL%>" <%=sCallTyp != null && sCallTyp.equals(CRMConstants.CALL_RESULT_INBOUNDCALL) ? "selected" : "none"%>>In-Bound Call</option>
                                <option value="<%=CRMConstants.CALL_RESULT_OUTBOUNDCALL%>" <%=sCallTyp != null && sCallTyp.equals(CRMConstants.CALL_RESULT_OUTBOUNDCALL) ? "selected" : "none"%>>Out-Bound Call</option>
                                <option value="<%=CRMConstants.CALL_RESULT_MEETING%>" <%=sCallTyp != null && sCallTyp.equals(CRMConstants.CALL_RESULT_MEETING) ? "selected" : "none"%>>Meeting</option>
                                <option value="<%=CRMConstants.CALL_RESULT_NIGHTCALL%>" <%=sCallTyp != null && sCallTyp.equals(CRMConstants.CALL_RESULT_NIGHTCALL) ? "selected" : "none"%>>Night Call</option>
                                <option value="<%=CRMConstants.CALL_RESULT_INTERNET%>" <%=sCallTyp != null && sCallTyp.equals(CRMConstants.CALL_RESULT_INTERNET) ? "selected" : "none"%>>Internet</option>
                            </select>
                        </td>
                        <td  bgcolor="#dedede"  style="text-align:center" valign="middle">
                    ...
                    <tr>
                        <td STYLE="text-align:center" bgcolor="#dedede" colspan="3">
                            <button type="submit" onclick="JavaScript: search();"  STYLE="color: #27272A;font-size:15px;margin: 10px;font-weight:bold; width: 180px; "> Generate Report <IMG SRC="images/search.gif" ></button>  
