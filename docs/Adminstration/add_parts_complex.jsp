...
<HTML>
    ...
    String search,AddCode,add,isMainEq,AddName,addNew,tCost,code,name,price,count,cost,Mynote,del,scr,sBackToList,attachTask;
    String updateParts;
    if(stat.equals("En")){
    ...
    document.WORKSHOP_FORM.action = "<%=context%>/AssignedIssueServlet?op=saveconfigParts&projectID=<%=wbo.getAttribute("projectName").toString()%>&assignNote=<%=failureCode%>&uID=<%=uID%>&filterName=StatusProjectListAll&filteValue=<%=filterValue%>&page=spareparts&isDirectPrch=0&attachedOn="+txt;
    document.WORKSHOP_FORM.submit();  
    }else
    {
        document.WORKSHOP_FORM.action = "<%=context%>/AssignedIssueServlet?op=saveconfigParts&projectID=<%=wbo.getAttribute("projectName").toString()%>&assignNote=<%=failureCode%>&uID=<%=uID%>&filterName=StatusProjectListAll&filteValue=<%=filterValue%>&page=spareparts&isDirectPrch=0";
        document.WORKSHOP_FORM.submit();  
    }
}
    ...
    function set(){
      ...
      if(nowMode=="name"){
         for(var i=0;i<lookAheadArray.length;i++){ 
            if(lookAheadArray[i]==inputText[0].value){
                ...
                inputText[0].value=itemsCode[i];
                break;
            }
         }
       }    
       code[x].value= inputText[0].value;
       inputText[0].value="";
       nowRow=x;
...
function noCodeFound( ) {
 ...
 function clearField()
    {
       alert("test");
       var dd=document.getElementById('inputText');
       dd.value="";
    }
    function d()
    {
                ...
                <TABLE ALIGN="<%=align%>" DIR="<%=dir%>" ID="tab" CELLPADDING="0" CELLSPACING="0" width="650">
                    <TR>
                        ...
                        <TD CLASS="cell" bgcolor="goldenrod" STYLE="text-align:center;color:white;font-size:14" ROWSPAN="2" id="CellData" WIDTH="300">
                            ...
                            <input type="text" dir="ltr"  class="head" autocomplete="off" value="Auto Select " ONCLICK="JavaScript: clearField();"  name="inputText" ID="inputText">
                        </TD>
