...
<%
    ...
    Vector userDeptsList = (Vector) request.getAttribute("userDeptsList");
...
<HTML>
            ...
            function submitForm()
            {
                if (checkGroup() == true) {
                    if (checkRadioDefault() == true) {
                        document.USER_DEPTS_FORM.action = "<%=context%>/UsersServlet?op=saveUsersDeptsConfig";
                        document.USER_DEPTS_FORM.submit();
                    } else {
                        alert("Select default department for user.");
                    }
                } else {
                    alert("Select at least one department for user");
                }
            }

            function cancelForm()
            {
                document.USER_DEPTS_FORM.action = "<%=context%>/ListerServlet?op=ListUsers";
                document.USER_DEPTS_FORM.submit();
            }
    ...
    <BODY>
        <FORM NAME="USER_DEPTS_FORM" METHOD="POST">
            ...
            <fieldset class="set" align="center">
                <legend align="center">
                    <table align="<%=align%>" dir=<%=dir%>>
                        <tr>
                            <td class="td">
                                <font color="blue" size="6">تعديل ارتباط الأدارات</font>
                            </td>
                        </tr>
                    </table>
                </legend>
                ...
                <%if (userDeptsList.size() > 0 && userDeptsList != null) {%>
                <TABLE CLASS="blueBorder" style="border-color: silver; border-right-WIDTH:1px;" WIDTH="600" CELLPADDING="0" CELLSPACING="0"  ALIGN="center" DIR="<%=dir%>">
                    ...
                    <input type="hidden" name="rows" id="rows" value="<%=userDeptsList.size()%>">

                    <%
                        for (int i = 0; i < userDeptsList.size(); i++) {
                            WebBusinessObject wbo = (WebBusinessObject) userDeptsList.get(i);
                    %>
                    <TR>
                        <TD  CLASS="cell">
                            <%=i + 1%>
                        ...
                        <TD  CLASS="cell">
                            ...
                            %>
                            <INPUT TYPE="RADIO" NAME="isDefault" ID="isDefault<%=i%>" value="<%=wbo.getAttribute("projectID")%>" checked>
                            <%} else {%>
                            <INPUT TYPE="RADIO" NAME="isDefault" ID="isDefault<%=i%>" value="<%=wbo.getAttribute("projectID")%>" disabled>
                            <%}%>
                        </TD>
                        ...
                        <TD  CLASS="cell">
                            ...
                            %>
                            <INPUT TYPE="CHECKBOX" NAME="userGroups" ONCLICK="checkDefault(<%=i%>)" value ="<%=wbo.getAttribute("projectID")%>" ID="group<%=i%>" checked> 
                            <%} else {%>
                            <INPUT TYPE="CHECKBOX" NAME="userGroups" ONCLICK="checkDefault(<%=i%>)" value ="<%=wbo.getAttribute("projectID")%>" ID="group<%=i%>"> 
                            <%}%>
                        </TD>
                    </TR>
                    <%
